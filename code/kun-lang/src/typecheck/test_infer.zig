...
test "infer: function effect detection" {
    const cases = [_]struct {
        name: []const u8,
        buildBody: *const fn (alloc: std.mem.Allocator) !*ast.Expr,
        expect_effect: bool,
        expect_type: u32,
    }{
        .{
            .name = "int literal",
            .buildBody = struct {
                fn f(alloc: std.mem.Allocator) !*ast.Expr {
    ...
    for (cases) |c| {
        ...
        try std.testing.expectEqual(c.expect_type, result[0].kind.function_def.type_);
    }
}
