...
test "json parse simple object" {
    ...
    const args = [_]Value{Value{ .string = "{\"key\":42}" }};
    const result = jsonFromStringImpl(&env, &args);
    try std.testing.expect(result == .adt);
    try std.testing.expectEqual(@as(u8, 0), result.adt.tag); // Ok
    const map = result.adt.payload.*;
    try std.testing.expect(map == .map);
    try std.testing.expect(map.map.len >= 1);
}
...
test "json parse invalid returns err" {
    const allocator = std.testing.allocator;
    var env = RuntimeEnv{ .frame = undefined, .primitives = .{ .bindings = &.{} }, .allocator = allocator };
    const args = [_]Value{Value{ .string = "not json" }};
    const result = jsonFromStringImpl(&env, &args);
    try std.testing.expect(result == .adt);
    try std.testing.expectEqual(@as(u8, 1), result.adt.tag); // Err
}

test "json parse null" {
    const allocator = std.testing.allocator;
    var env = RuntimeEnv{ .frame = undefined, .primitives = .{ .bindings = &.{} }, .allocator = allocator };
    const args = [_]Value{Value{ .string = "null" }};
    const result = jsonFromStringImpl(&env, &args);
    try std.testing.expect(result == .adt);
    try std.testing.expectEqual(@as(u8, 0), result.adt.tag);
    try std.testing.expect(result.adt.payload.* == .nil);
}

test "json toString integer" {
    const allocator = std.testing.allocator;
    var env = RuntimeEnv{ .frame = undefined, .primitives = .{ .bindings = &.{} }, .allocator = allocator };
    const args = [_]Value{Value{ .int = 42 }};
    const result = jsonToStringImpl(&env, &args);
    try std.testing.expect(result == .adt);
    try std.testing.expectEqual(@as(u8, 0), result.adt.tag);
    const s = result.adt.payload.*;
