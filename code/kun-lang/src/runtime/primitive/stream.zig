...
test "stream range produces sequence" {
    ...
    var env = RuntimeEnv{ .frame = undefined, .primitives = .{ .bindings = &.{} }, .allocator = allocator };
    const args = [_]Value{ Value{ .int = 1 }, Value{ .int = 5 }, Value{ .int = 1 } };
    const result = streamRangeImpl(&env, &args);
    try std.testing.expect(result == .stream);
    const list_val = streamToListImpl(&env, &.{result});
    try std.testing.expect(list_val == .list);
    try std.testing.expectEqual(@as(usize, 4), list_val.list.items.len);
}

test "stream fromList round trip" {
    const allocator = std.testing.allocator;
    var env = RuntimeEnv{ .frame = undefined, .primitives = .{ .bindings = &.{} }, .allocator = allocator };
    const items = try allocator.alloc(Value, 3);
    items[0] = Value{ .int = 10 };
    items[1] = Value{ .int = 20 };
    items[2] = Value{ .int = 30 };
    const list = Value{ .list = .{ .items = items, .cap = 3 } };
    const stream_val = streamFromListImpl(&env, &.{list});
    try std.testing.expect(stream_val == .stream);
    const result = streamToListImpl(&env, &.{stream_val});
...
test "streamToList empty stream" {
    const allocator = std.testing.allocator;
    var env = RuntimeEnv{ .frame = undefined, .primitives = .{ .bindings = &.{} }, .allocator = allocator };
    const empty_list = Value{ .list = .{ .items = &.{}, .cap = 0 } };
    const stream_val = streamFromListImpl(&env, &.{empty_list});
    const result = streamToListImpl(&env, &.{stream_val});
    try std.testing.expect(result == .list);
