...
- 任何需求理解、设计决策、架构变更都必须以文件形式记录到 `docs/` 对应目录
...
2. 阅读相关的 owner docs（`docs/context/`、`docs/design/`、`docs/architecture/`）
3. 检查 `docs/skills/` 中是否有适用的技能
4. 记录任务路由决策

## 操作规则

1. **文档先行**：修改代码前先更新对应的设计/架构文档
2. **输入→讨论→需求流程**：所有新功能遵循 `docs/input/` → `docs/discussions/` → `docs/requirements/` 流程
3. **先计划后实现**：涉及多模块或超过 5 个文件的变更必须先写计划（`docs/plans/`）
4. **最小代码注释**：代码应自文档化，注释仅用于解释"为什么"而非"是什么"
5. **归档意识**：版本迭代前，将当前版本文档迁移到 `docs/archive/<version>/`
6. **技能约束**：使用 `docs/skills/` 中的提示词模板，但必须结合 owner docs 使用
7. **错误模式升级**：连续出现同类错误时必须记录到 `docs/bugs/` 并升级处理
...
AI 自治策略定义在 `docs/context/ai-autonomy-policy.md`，包含以下级别：
...
- 所有输出文件必须保存在 `/home/z/my-project/` 目录下
- 文档使用 Markdown 格式，遵循 `docs/skills/writing-conventions.md`
