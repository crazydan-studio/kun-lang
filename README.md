...
Kun（鲲）是一款函数式、强静态类型、代数数据类型、结构化 + 不可变数据、表达式导向语法、高性能的 Linux 脚本语言。支持以函数形式调用任意 Linux 命令，提供强大的命令编程和组合能力，从根本上消除 Shell 脚本所存在的各种问题。
...
项目文档使用 VitePress 构建，可通过 [tools/build-docs.sh](tools/build-docs.sh) 构建，通过 [tools/serve-docs.sh](tools/serve-docs.sh) 本地预览。
