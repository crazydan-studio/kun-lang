...
效应函数涵盖以下命名空间的所有函数：`IO.*`、`File.*`、`Env.*`、`Process.*`、`Task.*`、`Random.*`，以及 `Signal.on`（信号注册）。`Cmd.<bin>` 构造 `Command` 值及 `Cmd` 单值修饰器（`Cmd.withEnv`、`Cmd.withWorkDir`、`Cmd.withStdin`、`Cmd.withRawOpt`、`Cmd.mergeStderr`、`Cmd.withRunAs`——接收 `Command` 并返回 `Command`）及条件组合器（`Cmd.andThen`、`Cmd.orElse`——接收两个 `Command` 返回 `Command`）为纯操作。`Cmd.pipe` 为管道组合器（接收 `List Command` 返回 `Command`，不立即执行，纯操作）。`Cmd.pipe?`/`Cmd.pipe!` 为立即执行管道（接收 `List Command` 并立即 fork-exec 整个管道链，`?` 返回 `Result`，`!` 失败 panic——效应函数）。`Cmd.<bin>?`/`Cmd.<bin>!`、`Cmd.pipe?`/`Cmd.pipe!`、`Cmd.timeout`、`Cmd.retry`、`Cmd.execSafe`、`Cmd.which`（PATH 查找需文件系统访问）、`Cmd.exec`（显式执行 Command 并丢弃输出）为效应函数。
