...
| `let ... in` 绑定 | 延迟 | `let` 与 `in` 之间的绑定仅在 `in` 之后被引用时才真正求值，绑定时不求值。`let ... in` 不可出现在 `do` 上下文——同一函数 scope 内 `do` 与 `let` 互斥。`let` 绑定的表达式必须是纯的——不得包含任何效应函数调用、定义，及效应命名空间函数的引用。效应代码必须使用 `do` 块内的 `=` 绑定 |
| Record/List/Map/Set 字面量 | 严格 | 所有元素/字段在构造前求值 |
| Record 字段访问/更新 | 严格 | 被访问的 Record 表达式先求值，然后按编译期偏移量读取/更新字段 |
| `defer` 注册 | — | 非求值操作——在最近外层 `do` 块退出时按 LIFO 逆序执行。注册时不求值；执行时的求值策略与正常表达式相同。`defer` 仅能在 `do` 块中使用 |
| `do` 块 | 严格 | 按顺序执行每条语句，结果固定为 `Unit`；`do in` 返回 `in` 之后表达式的值（必须为非 `Unit`）。Unbound `case`/`if` 在 `do` 上下文中的分支为隐式 `do`，结果均为 `Unit` |
...
`do` 块按顺序执行效应操作，结果固定为 `Unit`。含 `do` 块的函数通过编译器 AST 标记自动识别为效应函数。签名中声明了 `(a -> b)!` 效应回调参数的函数同样自动标记为效应函数。纯函数（无 `do` 块、无 `!` 参数）不能调用效应函数——编译期拒绝。

效应函数的识别采用**三层机制**：
1. **`do` 块 AST 扫描**：函数体含 `do` 块 → 自动推断为效应函数
2. **`(a -> b)!` 回调标注**：签名中声明了 `!` 参数的函数 → 标记为效应函数
3. **Primitive 函数效应标记**（Phase 3+ `PrimitiveBinding.is_effect`）：标准库 Primitive 函数的效应性由编译期常量表记录，不引入 `effect` 关键字

> **MVP（v0.1）过渡方案**：Primitive 函数表在 Phase 3+ 才实现。MVP 阶段 Primitive 函数的效应性通过硬编码命名空间前缀匹配识别——以下命名空间的所有函数均为效应函数：`IO.*`、`File.*`、`Env.*`、`Process.*`、`Task.*`、`Random.*`，以及 `Signal.on`。`Cmd.<bin>` 构造 `Command` 值及 `Cmd` 单值修饰器（`Cmd.withEnv`、`Cmd.withWorkDir`、`Cmd.withStdin`、`Cmd.withRawOpt`、`Cmd.mergeStderr`、`Cmd.withRunAs`）及条件组合器（`Cmd.andThen`、`Cmd.orElse`）为纯操作。`Cmd.pipe` 为管道组合器，纯操作。`Cmd.<bin>?`/`Cmd.<bin>!`、`Cmd.pipe?`/`Cmd.pipe!`、`Cmd.timeout`、`Cmd.retry`、`Cmd.execSafe`、`Cmd.which`、`Cmd.exec` 为效应函数。Phase 3+ 用 `PrimitiveBinding.is_effect: bool` 字段替代硬编码列表，保持 `do` 块扫描 + `!` 标注两种机制不变。
...
效应函数的函数体必须显式以 `do` 或 `do in` 包裹，无论单语句还是多语句。纯函数体为 `let in`（单一表达式时可省略）。匿名函数同样遵循此约束。

`do` 上下文内，未被值绑定、不作为函数返回值的 `case`/`if`（unbound）的各分支为隐式 `do`，结果均为 `Unit`。隐式 `do` 分支内 `defer` 的作用域为该分支自身的隐式 `do`。详见[语法设计](../design/syntax.md#单一表达式范式)。

`do` 块内使用 `=` 绑定值（严格求值，按声明顺序）。`do in` 形式在副作用执行后返回值——`in` 之后表达式的结果即为整个 `do in` 的结果，`in` 处于效应上下文内。语法细节见 [`syntax.md`](../design/syntax.md) do 块章节。
...
3. **`let ... in` 交互**：`let ... in` 不可出现在 `do` 上下文（同一函数 scope 内 `do` 与 `let` 互斥），因此 Stream 不会被 `let` 延迟绑定——消除了一类分析歧义。
...
在类型合一的同时，效应检查器（Effect Checker）扫描 AST 中的 `do` 块、效应命名空间调用和 `!` 参数声明。完整规则见 [类型系统设计 > 效应检查](../design/type-system.md#效应检查)，以下列出实现要点：
...
- 验证 `Cmd.<bin>?`/`Cmd.<bin>!`/`Cmd.pipe?`/`Cmd.pipe!`/`Cmd.timeout`/`Cmd.retry`/`Cmd.execSafe`/`Cmd.which`/`Cmd.exec` 仅在 `do` 块内使用
- Lambda 含有效应函数调用时，要求该 lambda 在 `do` 块内定义
- 验证 `do` 块内未被消费的 `Command` 值
- 验证 `let in` 内无效应操作痕迹（调用、定义、引用）
- 验证 `do`/`do in`/`let in` body 非空
- 验证 `do in` 的 `in` 结果非 `Unit`
- 验证同一 scope 内 `do` 与 `let` 不互相嵌套
- 识别 unbound `case`/`if` 分支为隐式 `do`
- 告警：无效应调用的 `do`/隐式分支、未消费的纯表达式/绑定等
...
const PrimitiveBinding = struct {
    ...
    is_effect: bool,     // 效应检查器快速路径（冗余于 signature 变体判别，但避免 TypeEnv 解引用）
};
```
...
| 版本 | 变更 |
|------|------|
| 2026.07.16 | 沙箱加固（参考 Z-Jail 分层防御思路）：父进程层新增 `PR_SET_DUMPABLE=0`（禁 core dump + 锁 `/proc/self/mem`）、capabilities 显式清零（`capset` + securebits `_LOCKED` 锁定，条件性保留 `CAP_SETUID`/`CAP_SETGID` 供 `Cmd.withRunAs` 使用，子进程降权后全清）、ns 兜底模式新增 `CLONE_NEWIPC`（隔离 SysV IPC）；子进程层新增 fd 清理（fd scrub，fork 后关闭 fd≥3 防止特权 fd 逃逸）；Command 执行契约同步更新 fd 清理与 withRunAs 后 capset 全清步骤；新增 Z-Jail 参考说明 |
