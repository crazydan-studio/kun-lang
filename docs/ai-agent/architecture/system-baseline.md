...
| 层 | 技术选择 | 说明 |
|---|---|---|
| 宿主语言 | Zig 0.16.0 | 高性能、无 hidden control flow、直接操作内存 |
...
2. **AST 求值**：递归求值 Typed AST 节点。求值器采用 Zig 0.16 的**标记 switch**（labeled switch）实现节点分发——每个 AST 节点类型的求值逻辑为独立分支，利用 `continue :eval` 实现尾递归消除和分支预测优化。标记 switch 使 CPU 分支预测器能够将各节点类型的调度关联到独立的分支指令上，在热路径（如 Stream 消费循环中的逐元素求值）中获得显著性能提升
...
const Expr = union(enum) {
    ...
    // 单表达式与效应
    let_in_block: struct { stmts: []const LetInStmt, result: *Expr },   // `let in`/`do` 统一单表达式（`do <body>` ≡ `let <body> in ()`）
    defer_stmt: struct { expr: *Expr },

    // 命令
    cmd_literal: struct { name: []const u8, subcommands: []const []const u8, options: ?*Expr, args: []const *Expr },   // `cmd` 字面量
...
const Type = union(enum) {
    ...
    function: struct { param: TypeId, result: TypeId, effects: EffectSetId },   // 统一函数类型，效应集作为独立组成部分
    // 标准库类型（编译器内置支持但非基础类型）
    decimal_t,
    command_t,   // `cmd` 字面量构造的 Command 值，不透明类型
    datetime_t,
};
```
...
> `Regex` 的运行时表示为指向 [zig-regex](https://github.com/zig-users/zig-regex) 编译后正则对象的指针。`Command` 为 `cmd` 字面量构造的不透明值，内部结构为编译器实现细节；`Decimal` 以尾数+指数二元组表示。
>
...
ADT 在运行时表示为带标记的联合体（tagged union），tag 使用 `uint8_t`。Zig 0.16 中，packed union/struct 支持原生等值比较，求值器在模式匹配时对 tag 的比较可直接使用 `packed struct` 的 `==` 运算符（等价于底层整数的单指令比较），无需手动提取 tag 字节后比较。
...
const CommandPayload = struct { tag: u8, payload: [32]u8 };
```
...
Arena 分配器特性：线性分配（bump allocation），无释放操作；Arena 在阶段结束时整体销毁。Zig 0.16 的 `ArenaAllocator` 为线程安全且无锁（lock-free）实现——若未来引入并发特性（如 `Task.spawn`），Arena 不会成为并行度的瓶颈。
...
标准库模块的实现分为两类：纯 Kun 实现（`.kun` 文件，用语言自身编写）和 Primitive 实现（Zig 原生函数，注册到内置 Primitive 函数表）。编译器语法级结构（`cmd` 字面量、`let in`/`defer`、`case`、`?T`/`Nil`、`|>` 管道等）不在此列——它们由编译器直接解析和代码生成。
