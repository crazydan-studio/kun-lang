...
效应函数涵盖以下命名空间的所有函数：`IO.*`、`File.*`、`Env.*`、`Process.*`、`Sys.*`、`Task.*`、`Random.*`，以及 `Signal.on`（信号注册）。`Cmd.<bin>` 构造 `Command` 值及 `Cmd` 单值修饰器（`Cmd.withEnv`、`Cmd.withCwd`、`Cmd.withStdin`、`Cmd.withRawOpt`、`Cmd.mergeStderr`、`Cmd.withRunAs`——接收 `Command` 并返回 `Command`）为纯操作。`Cmd.pipe`/`Cmd.pipe?` 为管道组合器（接收 `List Command` 返回 `Command`，不立即执行）。`Cmd.<bin>?`、`Cmd.pipe?`、`Cmd.timeout`、`Cmd.retry`、`Cmd.which`（PATH 查找需文件系统访问）、`Cmd.exec`（显式执行 Command 并丢弃输出）为效应函数。
