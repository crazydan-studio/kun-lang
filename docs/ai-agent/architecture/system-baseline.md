...
5. **类型检查**：对 AST 进行类型推断和检查，生成带类型标注的 Typed AST；同时执行效应检查（函数效应集由体内所有效应调用的并集推导，标注在函数类型上 `a -> b ! {E}`）
...
3. **沙箱安装**：
   - 解析 CLI 安全参数（`--allow-path`、`--allow-net`、`--allow-ffi`、`--no-sandbox`、`--force`、`--env=`、`--cpu-limit`、`--mem-limit`、`--audit`）
...
```
入口解析 → AST 求值 → `let in`/`do` 单表达式求值 → 命令调用
```

1. **入口解析**：按入口规则确定执行起点（`main : List String -> Unit ! {IO, File, Cmd, ...}`，类型标注可选）
2. **AST 求值**：递归求值 Typed AST 节点。求值器采用 Rust 的 `match` 表达式实现节点分发——每个 AST 节点类型的求值逻辑为独立分支，利用尾递归或显式栈循环实现求值。编译器对 `match` 的分支优化与内联指令生成能让 CPU 分支预测器将各节点类型的调度关联到独立的分支指令上，在热路径（如 Stream 消费循环中的逐元素求值）中获得显著性能提升
3. **`let in`/`do` 单表达式求值**：`let in`/`do` 块按顺序执行语句（绑定 / 效应调用 / 纯表达式），立即求值，`defer` 在退出时按 LIFO 逆序执行。`let in` 与 `do` 统一所有多语句形式：返回值的用 `let <body> in <expr>`，返回 `Unit` 的用 `do <body>`（`do` 是 `let <body> in ()` 的语法糖，可紧跟 `->`）

defer 的作用域为其**最近的外层 `let in` 块**——退出该 `let in` 块时（正常返回或 panic）该块内注册的所有 defer 按 LIFO 逆序执行。嵌套 `let in` 块各自管理独立的 defer 链：

```kun
let          // outer let in
  let        // inner let in
    defer inner_cleanup  // 在 inner let in 退出时执行
  in
    ()
  // inner_cleanup 已在此时执行完
  defer outer_cleanup  // 在 outer let in 退出时执行
in
  ()
```

4. **命令调用**：`cmd` 字面量构造 `Command` 值，通过显式执行函数（`Cmd.exec`/`Cmd.execSafe`/`Cmd.stream`）触发 fork-exec。`|>` 退化为纯管道，不再隐式触发执行
...
1. **defer 执行**：按 LIFO 逆序执行当前 `let in` 块及其所有外层 `let in` 块注册的所有 `defer` 块
2. **资源释放**：
   - 关闭所有打开的文件描述符
...
| `let in` 绑定 | 立即 | `let` 与 `in` 之间的绑定在声明点立即求值（call-by-value），按声明顺序。`let in` 与 `do` 统一所有多语句形式：返回值用 `let <body> in <expr>`，返回 `Unit` 用 `do <body>`（`do` 是 `let <body> in ()` 的语法糖，可紧跟 `->`） |
| `Lazy.lazy` | 显式惰性 | `Lazy.lazy : (Unit -> a) -> Lazy a` 构造 thunk，包装表达式为闭包；`Lazy.force : Lazy a -> a` 强制求值并 memoize 结果。详见 [Lazy 模块](../design/standard-library.md#lazy-显式惰性) |
| Record/List/Map/Set 字面量 | 严格 | 所有元素/字段在构造前求值 |
| Record 字段访问/更新 | 严格 | 被访问的 Record 表达式先求值，然后按编译期偏移量读取/更新字段 |
| `defer` 注册 | — | 非求值操作——在最近外层 `let in` 块退出时按 LIFO 逆序执行。注册时不求值；执行时的求值策略与正常表达式相同。`defer` 仅能在 `let in` 块中使用 |
| 单表达式 | 严格 | `let in`/`do` 统一所有多语句形式；按顺序执行每条语句；返回 `in` 之后表达式的值；返回 `Unit` 时用 `do <body>`（`do` 是 `let <body> in ()` 的语法糖，可紧跟 `->`）。Unbound `case`/`if` 在 `let in` 上下文中的分支为隐式 `let in`，结果视为 `Unit` |

显式惰性通过 `Lazy` 模块实现：`Lazy.lazy` 构造 thunk（包装表达式为 `(Unit -> a)` 闭包），`Lazy.force` 强制求值并 memoize 结果——确保 thunk 内的表达式仅求值一次。`let in` 绑定的表达式立即求值并绑定到名字；若需延迟求值，显式使用 `Lazy.lazy` 包装。`let in` 体内可包含效应调用（无绑定的效应语句立即执行，结果丢弃）。
...
TCO 为未来候选特性。若未来引入，将对尾递归路径豁免深度计数器。

### `let in` 块与效应函数

`let in` 块按顺序执行语句（绑定 / 效应调用 / 纯表达式），立即求值，返回 `in` 之后表达式的值（返回 `Unit` 时用 `do <body>`，`do` 是 `let <body> in ()` 的语法糖）。函数的效应集由体内所有效应调用的并集推导，标注在函数类型上 `a -> b ! {E}`。纯函数的效应集为空集（`! {}`，可省略）——编译期拒绝纯函数调用效应函数。

效应函数的识别采用**效应集推导机制**：
1. **函数签名效应集**：函数类型的效应部分 `! {E}` 是函数调用者必须接纳的效应集
2. **效应调用收集**：函数体内所有效应操作调用（`Effect.op`、`Cmd.exec`、`IO.println` 等）的并集为函数体效应集
3. **Primitive 函数效应标记**：标准库 Primitive 函数的效应性由编译期 `PrimitiveBinding.is_effect` 字段记录。每个 Primitive 绑定包含预解析的函数类型签名，其中效应部分直接参与效应集推导

> **过渡方案**：Primitive 函数表使用 `PrimitiveBinding.is_effect: bool` 字段标识效应函数。该字段冗余于 `signature` 中的效应集判别（避免 TypeEnv 解引用，快速路径）。所有内置效应的 Primitive 函数（`IO.*`、`File.*`、`Cmd.exec`/`Cmd.execSafe`/`Cmd.stream`/`Cmd.which`、`Random.*`、`DateTime.*`、`Signal.on`、`FFI.call`）均标记为效应函数；纯函数（如 `Cmd.withEnv`、`Cmd.withWorkDir`、`pipe` 等 Command 修饰函数与组合器）不标记。

函数体使用 `let in` 统一包裹多语句。单语句函数可直接写表达式（无 `let in` 包裹）。匿名 lambda 同样使用 `let in`。

`let in` 上下文内，未被值绑定、不作为函数返回值的 `case`/`if`（unbound）的各分支为隐式 `let in`，结果视为 `Unit`。隐式分支内 `defer` 的作用域为该分支自身的隐式 `let in`。详见[语法设计 - 单表达式](../design/syntax.md#单表达式-let-in-与-do)。

`let in` 块内使用 `=` 绑定值（立即求值，按声明顺序）。`in` 之后表达式的结果即为整个 `let in` 块的结果。语法细节见 [`syntax.md`](../design/syntax.md) 单表达式章节。

### Command 执行模型

`cmd` 字面量构造 `Command` 值——不立即执行。执行必须显式调用执行函数（`Cmd.exec`/`Cmd.execSafe`/`Cmd.stream`），详见 [OS 命令调用机制](../design/command-system.md#command-执行)。未被消费的 `Command` 值是编译错误。

`Cmd.exec : Command -> Unit ! {Cmd}` 为阻塞调用——内部执行 fork → exec → waitpid，子进程退出后才返回。需错误处理的场景使用 `Cmd.execSafe : Command -> Result (Stream String) CommandError ! {Cmd}`；需输出流但不需错误处理的场景使用 `Cmd.stream : Command -> Stream String ! {Cmd}`。此语义确保 `let in` 块内的 `Cmd.exec` 调用间有确定的执行顺序。
...
```rust
enum Stream {
    Cmd { fd: i32, pid: i32, buf: Vec<u8> },
    Mapped { upstream: Box<Stream>, f: FnPtr },
    Filtered { upstream: Box<Stream>, pred: FnPtr },
    Taken { upstream: Box<Stream>, remaining: usize },
    Dropped { upstream: Box<Stream>, remaining: usize },
    Lines { upstream: Box<Stream>, buf: Vec<u8>, pos: usize, max_len: usize },
    ParseMapped { upstream: Box<Stream>, f: FnPtr },
    ParseMappedKeep { upstream: Box<Stream>, f: FnPtr },
}
```
...
编译器对 `let in` 块内通过 `cmd` 字面量、`pipe` 或 `Stream.*` 构造操作产生的 Stream 值执行 **AST 级穷举消费分析**（非全程序流敏感分析）：

1. **作用域**：分析的粒度为单个 `let in` 块。Stream 值跨 `let in` 块边界传递、作为函数参数传递或作为返回值时，视为"已消费"——不追踪跨边界别名。
2. **检测规则**：`let in` 块末尾检查每个在此块内构造且绑定到变量的 Stream 值是否存在消费路径（`toList`/`iter`/`fold`/`string`/`bytes`）。条件消费（`if`/`case` 分支）的所有分支均需消费——缺失分支编译期报错。
3. **`let in`/`do` 块为统一多语句形式**：Stream 不会跨 `let in` 与 `do` 边界传递（`do` 是 `let <body> in ()` 的语法糖，统一为单表达式），分析粒度明确。
...
此策略确保长时间运行的管道（如 `cmd find {} [] |> Cmd.stream |> Stream.lines |> Stream.filter`）不会因缓冲区满而死锁。
...
对阶段一生成的约束集，使用**约束式 HM 合一求解器**（与 [类型系统 - 类型推断](../design/type-system.md#类型推断) 描述一致）逐条解析：
...
- 验证纯函数（`! {}`）体中无效应函数调用
- 验证调用者效应集包含被调用者效应集（子集关系）
- 验证效应调用 `Effect.op` 的效应名在函数效应集中（或被 handler 消解）
- 验证 `Cmd.exec`/`Cmd.execSafe`/`Cmd.stream`/`Cmd.which` 等效应函数的调用上下文有 `Cmd` 效应
...
> **注**：`code/kun-lang/` 实现已撤销（commit `559180a`，2026-07-17），本节描述的 Typed AST 结构为设计规范，待重新实现时遵循。
...
```rust
type TypeId = u32;  // 类型环境中的索引

struct TypedExpr<'a> {
    expr: &'a Expr<'a>,   // 指向原始 AST 节点的引用（Arena 分配）
    ty: TypeId,           // 该表达式的推断类型
}
```
...
enum Expr<'a> {
    ...
    // 变量与绑定
    Variable { name: &'a str, index: u32 },
    LetIn { name: &'a str, value: Box<Expr<'a>>, body: Box<Expr<'a>> },
    LetRec { bindings: &'a [LetBinding<'a>], body: Box<Expr<'a>> },

    // Lambda 与函数应用
    Lambda { params: &'a [Param], body: Box<Expr<'a>> },
    Apply { func: Box<Expr<'a>>, arg: Box<Expr<'a>> },

    // 控制流
    If { cond: Box<Expr<'a>>, then: Box<Expr<'a>>, else_: Box<Expr<'a>> },
    Case { scrutinee: Box<Expr<'a>>, branches: &'a [Branch<'a>] },

    // 复合类型
    RecordLiteral { fields: &'a [RecordFieldExpr<'a>] },
    RecordAccess { record: Box<Expr<'a>>, field: &'a str },
    RecordUpdate { record: Box<Expr<'a>>, updates: &'a [RecordFieldExpr<'a>] },
    TupleLiteral { elements: &'a [Expr<'a>] },
    ListLiteral { elements: &'a [Expr<'a>] },
    MapLiteral { entries: &'a [MapEntry<'a>] },
    SetLiteral { elements: &'a [Expr<'a>] },

    // 运算符
    BinaryOp { op: BinOp, lhs: Box<Expr<'a>>, rhs: Box<Expr<'a>> },
    UnaryOp { op: UnaryOp, expr: Box<Expr<'a>> },
    PipeOp { lhs: Box<Expr<'a>>, rhs: Box<Expr<'a>> },

    // 单表达式与效应
    LetInBlock { stmts: &'a [LetInStmt<'a>], result: Box<Expr<'a>> },   // `let in`/`do` 统一单表达式（`do <body>` ≡ `let <body> in ()`）
    DeferStmt { expr: Box<Expr<'a>> },

    // 命令
    CmdLiteral { name: &'a str, subcommands: &'a [&'a str], options: Option<Box<Expr<'a>>>, args: &'a [Expr<'a>] },   // `cmd` 字面量

    // 模式匹配辅助
    PatternMatch { expr: Box<Expr<'a>>, pattern: Pattern<'a> },
}
```
...
enum Type {
    ...
    Adt { name: String, variants: Vec<ADTVariant> },
    Nilable(TypeId),   // Nilable T — 编译器内置 ADT 的快捷表示，等价于 adt{ Some(T), Nil }
    Function { param: TypeId, result: TypeId, effects: EffectSetId },   // 统一函数类型，效应集作为独立组成部分
    // 标准库类型（编译器内置支持但非基础类型）
    DecimalT,
    CommandT,   // `cmd` 字面量构造的 Command 值，不透明类型
    DatetimeT,
}
```

> `command_t` 为不透明单元变体——所有 `cmd` 字面量产生的 Command 值在类型系统中等价（结构等价）。编译器不对 Command 值的特定命令名或选项类型做类型级区分——选项错误在运行时 fork/exec 阶段由命令本身报告。若后续版本引入类型化命令模块，`command_t` 可扩展为携带 `<bin>_options` 参数的变体。
...
5  │ compute = \n -> File.readString p"/etc/config"
   │                  ^^^^^^^^^^^^^^^
  ──┤
  Hint: 将 compute 的效应集改为含 `File`，或改为返回常量的纯函数
```
...
命令执行阶段的语义化错误类型，完整定义见 [`standard-library.md`](../design/standard-library.md)。包含 `NotFound`、`PermissionDenied`、`CommandFailed`、`KilledBySignal`、`IoError`、`PipeFailed`、`Timeout` 七个变体。
...
| 命令失败（`Cmd.exec`/`Cmd.stream`） | 运行时 | panic | unwind + defer 链 |
| 命令失败（`Cmd.execSafe`） | 运行时 | `Result` | 调用者通过 `case` 处理 |
| 同步 syscall 失败 | 运行时 | `Result` | 调用者通过 `case` 处理 |

### panic 语义

panic 触发 unwind 时，当前 `let in` 块的所有 `defer` 按 LIFO 逆序始终执行。未捕获的顶层 panic 退出码：
...
所有命令通过 `cmd` 字面量构造 `Command` 值，命令执行采用 fork-exec 子进程 + 管道捕获 stdout/stderr。语言层设计见 [OS 命令调用机制](../design/command-system.md)。
...
cmd <命令> <子命令>* <选项>? <位置参数>?
  ...
  │
  └── 运行时：fork-exec + Stream pipe 捕获（需显式调用 Cmd.exec/Cmd.execSafe/Cmd.stream 触发）
```
...
子进程 fork 后、exec 前执行以下清理以消除信号竞争与 fd 逃逸面：
...
`cmd` 字面量构造的 Command 的命令查找发生在**运行时**。编译时不检查命令是否存在。首次 PATH 解析成功后结果缓存（每次 `let in` 块入口刷新）。运行时未找到命令则触发 `NotFound` panic。

## 安全隔离

CLI 安全参数（`--allow-path`、`--allow-net`、`--allow-ffi`、`--no-sandbox`、`--force`、`--env=`、`--cpu-limit`、`--mem-limit`、`--audit`）的完整说明见 [`kun` CLI 工具](../design/kun-cli-tool.md#安全控制)。
...
> **capabilities 与 `Cmd.withRunAs` 的协调**：`Cmd.withRunAs` 需 `CAP_SETUID`/`CAP_SETGID` 执行 `setgid`/`setuid`。父进程层 capabilities 清零时，若检测到脚本使用了 `withRunAs`，则保留这两个 capability 于 inheritable 集；子进程在 withRunAs 降权完成后立即全清 capability，确保最终无 cap 状态。此协调保证 withRunAs 可用且最终安全态不变。

> **层次依赖**：Landlock 文件控制依赖 `PR_SET_NO_NEW_PRIVS` 生效；若 Landlock 不可用（内核 < 5.13），mount namespace 兜底提供文件系统隔离但不具备路径精细度。seccomp-BPF 在文件访问上完全依赖上两层——不阻止 `openat`/`read`/`write` 系统调用（这是 Kun 的基本功能前提）。三层安全均运作时，子进程在文件访问、网络、系统调用类型上形成完整隔离链。

> **`CLONE_NEWUSER` 不可用时的降级**：部分 Linux 发行版通过 sysctl 禁用非特权用户创建 user namespace（`kernel.unprivileged_userns_clone=0` 或 `user.max_user_namespaces=0`）。启动期检测此状态，若 `CLONE_NEWUSER` 不可用：
> - **特权进程（root 或 `CAP_SYS_ADMIN`）**：跳过 `CLONE_NEWUSER`，直接 `capset` + `CLONE_NEWNS`（特权进程本身有 `CAP_SYS_ADMIN`）
> - **非特权进程**：降级为 Landlock + seccomp 模式（无 `capset` 清零、无 mount namespace 目录隔离），通过 stderr 告警 `user namespaces disabled, sandbox degraded (no mount namespace isolation)`。Landlock 仍提供文件控制；`CLONE_NEWNET` 仍提供网络隔离（`CLONE_NEWNET` 不需要 `CAP_SYS_ADMIN` within user ns，但需要特权或 user ns——若 user ns 不可用且无特权，网络隔离也不可用，此时网络默认放行并告警）
> 
> 此降级策略确保 Kun 在所有 Linux 配置下可运行，但沙箱强度因环境而异：特权/user-ns 可用 → 完整沙箱；user-ns 禁用 + 非特权 → Landlock+seccomp（无 mount ns）；无 Landlock（内核 <5.13）+ user-ns 禁用 → 仅 seccomp + rlimit（告警）。
...
> **沙箱加固参考**：capabilities 显式清零、fd 清理（fd scrub）、`PR_SET_DUMPABLE=0`、IPC namespace 等加固措施参考了 [Z-Jail](https://github.com/Division-36/Z-Jail/) 的分层防御思路。Z-Jail 是一个零依赖的轻量级 Linux 沙箱（~73 KiB，7 层防御），其"分层安装 + 顺序不可逆"的设计原则启发了 Kun 补齐 `NO_NEW_PRIVS` 之外的防御缺口。但 Z-Jail 的 seccomp 白名单（仅 15 个 syscall）、强制 `pivot_root`、Truthimatics 判定引擎等核心特性不适用于脚本语言运行时（Kun 需要更宽的 syscall 面、Landlock 路径级控制、保留对真实文件系统的访问），故仅采纳其防御完整性细节。详见 [讨论记录](../discussions/discussion-z-jail-sandbox-hardening.md)。
...
> `Regex` 的运行时表示为指向 [`regex` crate](https://github.com/rust-lang/regex) 编译后正则对象的指针。`Command` 为 `cmd` 字面量构造的不透明值，内部结构为编译器实现细节；`Decimal` 以尾数+指数二元组表示。
>
> `Unit` 在 Record/Tuple 作为字段时占用 0 字节（编译器优化省略），但对齐视为 1（与 Rust/Zig 0 大小类型的对齐行为一致）。
>
...
ADT 在运行时表示为带标记的联合体（tagged union），tag 使用 `uint8_t`。Rust 中，`enum` 的 tag 比较可直接使用 `match`（编译器优化为单指令比较 tag 字节），求值器在模式匹配时无需手动提取 tag 字节后比较。

```c
// ADT 运行时布局：tag-first design
// Rust 实现中可用 enum + #[repr(u8, C)] 控制 tag 布局
// 匹配时直接比较 tag 字节：match adt { Variant(payload) => ... }
typedef struct {
    uint8_t tag;
    union {
...
Stream 在运行时表示为 Rust `enum`（tagged union，详见执行模型 Stream 惰性求值段）。其 C ABI 表示为一个不透明句柄——Kun 用户代码不直接操作 Stream 的内存布局，而是通过 `Stream.map`/`Stream.filter`/`Stream.toList` 等 API 消费：
...
enum Value<'a> {
    ...
    Duration(i64),
    Regex(&'a regex::Regex),
    Decimal { mantissa: i64, exponent: i32 },
    Command(CommandPayload),
    List(Array),
    Map(MapRepr),
    Set(SetRepr),
    Record(*mut std::ffi::c_void),    // 指向 Arena 上的 Record 内存
    Adt(ADTRepr),
    Closure(Closure),
    Stream(*mut StreamNode),          // 指向 tagged union Stream 节点
}

struct Array { ptr: *mut u8, len: u64, cap: u64 }
struct MapRepr { entries: *mut u8, len: u64, cap: u64 }
struct SetRepr { entries: *mut u8, len: u64, cap: u64 }
struct ADTRepr { tag: u8, payload: *mut u8 }
struct CommandPayload { tag: u8, payload: [u8; 32] }
```
...
Arena 分配器特性：线性分配（bump allocation），无释放操作；Arena 在阶段结束时整体销毁。Rust 的 `bumpalo::Bump` Arena 分配器为成熟实现，性能与 Zig `ArenaAllocator` 同级——若未来引入并发特性（如 `Task.spawn`），可通过 `bumpalo::Bump` 的线程局部实例或 `crossbeam-arena` 等并行 Arena 方案避免并行度瓶颈。

脚本模式（`kun` CLI）使用以上三层 Arena 模型。Kun Shell（未来版本）扩展为双 Arena + 绑定表三层内存模型以支持跨 REPL 求值的绑定持久化，完整设计见 [`kun-shell.md`](../design/kun-shell.md#内存模型)。
...
标准库模块的实现分为两类：纯 Kun 实现（`.kun` 文件，用语言自身编写）和 Primitive 实现（Rust 原生函数，注册到内置 Primitive 函数表）。编译器语法级结构（`cmd` 字面量、`let in`/`defer`、`case`、`?T`/`Nil`、`|>` 管道等）不在此列——它们由编译器直接解析和代码生成。
...
Primitive 函数表在编译期以 Rust `const`/`static` 生成（或 proc macro），运行时为静态只读数组：

```rust
type PrimitiveFn = fn(env: &mut RuntimeEnv, args: &Value) -> Value;

struct PrimitiveBinding {
    module: &'static str,      // 所属模块名，如 "IO"
    name: &'static str,        // 函数名，如 "println"
    fn_ptr: PrimitiveFn,       // Rust 实现函数指针
    is_effect: bool,           // 是否为效应函数（用于效应检查器，替代硬编码命名空间匹配）
}

struct PrimitiveTable {
    bindings: &'static [PrimitiveBinding],         // 按 module+name 排序的静态数组
    protected_modules: &'static [&'static str],    // 受保护模块名集合
}
```
...
运行时环境建立
  ...
  └── 注册 Primitive 函数表
        ├── 编译期常量表直接载入（无运行时分配）
        ├── 表引用标记为 const（不可再修改）
        └── 构建受保护模块名索引 {IO, File, Env, Process, Cmd, Random, Stream, Signal, Task}
```
...
import M
  ...
  │     │       ├── 对每个 export 函数 f：
  │     │       │     ├── Primitive 表中存在 (M, f) → 绑定 Rust 函数指针为函数体
...
2. 对每个函数查询 Primitive 表：存在 → 以 Rust 实现替换函数体；不存在 → 保留 .kun 中的实现
...
struct PrimitiveBinding {
    ...
    signature: TypeId,   // 函数类型（function 变体，含效应集）
    is_effect: bool,     // 效应检查器快速路径（冗余于 signature 中的效应集判别，但避免 TypeEnv 解引用）
}
```
...
| `Regex` | 几乎全部 Primitive | 基于 `regex` crate（Rust 纯实现正则引擎，性能与 zig-regex 同级） |
...
| `Path`、`Duration`、`Decimal`、`type` 单变体 ADT 模块 | 全部 PureKun | 基于已有基础类型的变换 |
