...
- `next` 返回 `?t`：非 `Nil` 表示有下一个元素，`Nil` 表示流已终止
...
| `?t`（Nilable） | `TaggedUnion { uint8_t is_nil, uint64_t value }` | 0=有值, 1=Nil |
...
对于没有 `.cmd.kun` 且无法直接 dlopen 的命令，运行时尝试 ptrace 适配：

1. **触发条件**：命令无 `.cmd.kun` 签名，且满足以下任一条件
   - 内置命令表未命中
   - 用户通过项目级 `.cmd.kun` 提供了签名定义
...
| POSIX 调用 | 执行底层系统调用（open、read、write、stat 等），所有 fd 创建时设 `O_CLOEXEC`。内核 5.6+ 时 `open` 类调用使用 `openat2()` + `RESOLVE_NO_SYMLINKS` + `RESOLVE_BENEATH` 防范 TOCTOU |
...
- **`.cmd.kun` 文件**：纯 Kun 语法定义命令签名，通过 Builder API 构造命令行参数
- **内建 Primitive**：核心命令（ls、cat、grep、find 等）以 Zig 内建实现
- **自动推断**：`kun cmd init` 从 man/--help 生成 `.cmd.kun` 骨架
