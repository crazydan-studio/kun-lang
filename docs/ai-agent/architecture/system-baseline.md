...
```
入口解析 → AST 求值 → `let in` 块表达式求值 → 命令调用
```
...
3. **`let in` 块表达式求值**：`let in` 块按顺序执行语句（绑定 / 效应调用 / 纯表达式），立即求值，`defer` 在退出时按 LIFO 逆序执行。`let in` 统一所有多语句形式，返回 `Unit` 时可省略 `in`（≡ `let <body> in ()`）
...
| `let in` 绑定 | 立即 | `let` 与 `in` 之间的绑定在声明点立即求值（call-by-value），按声明顺序。`let in` 统一所有多语句形式，废弃 `do`/`do in`。返回 `Unit` 时可省略 `in`（≡ `let <body> in ()`） |
...
| 块表达式 | 严格 | `let in` 统一所有多语句形式；按顺序执行每条语句；返回 `in` 之后表达式的值；返回 `Unit` 时可省略 `in`。Unbound `case`/`if` 在 `let in` 上下文中的分支为隐式 `let in`，结果视为 `Unit` |
...
`let in` 块按顺序执行语句（绑定 / 效应调用 / 纯表达式），立即求值，返回 `in` 之后表达式的值（返回 `Unit` 时可省略 `in`）。函数的效应集由体内所有效应调用的并集推导，标注在函数类型上 `a -> b ! {E}`。纯函数的效应集为空集（`! {}`，可省略）——编译期拒绝纯函数调用效应函数。
...
`let in` 上下文内，未被值绑定、不作为函数返回值的 `case`/`if`（unbound）的各分支为隐式 `let in`，结果视为 `Unit`。隐式分支内 `defer` 的作用域为该分支自身的隐式 `let in`。详见[语法设计 - 块表达式](../design/syntax.md#块表达式-let-in)。

`let in` 块内使用 `=` 绑定值（立即求值，按声明顺序）。`in` 之后表达式的结果即为整个 `let in` 块的结果。语法细节见 [`syntax.md`](../design/syntax.md) 块表达式章节。
...
3. **`let in` 块为统一多语句形式**：Stream 不会跨 `let in` 与 `do` 边界传递（已废弃 `do`），分析粒度明确。
...
const Expr = union(enum) {
    ...
    // 块表达式与效应
    let_in_block: struct { stmts: []const LetInStmt, result: *Expr },   // `let in` 统一块表达式
