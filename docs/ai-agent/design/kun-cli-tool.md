...
| `test` | `<module>_test.kun` 测试文件、`Test` 类型值、`--filter`/`--timeout`/`--parallel`/`--fail-fast`/`--report`（推迟 v1.2） |
...
### `main` 与 `Test.body` 的 `handle with` 限制

`handle with` 表达式**仅在 `main` 函数与 `Test` 值的 `body` 字段内可用**——业务函数只声明效应不消解，效应冒泡到入口级上下文集中消解。

- **`main`**：程序入口，允许消解用户效应（内置效应自动注入默认 handler）
- **`Test.body`**：`Test` 类型值（`type Test = Test { name, description, timeout, body, with }`）的 `body` 字段，由 `kun test` 运行器在入口级上下文执行，允许使用 `handle with`；用户效应通常通过 `Test.with` 字段声明式消解（详见 [单元测试设计](testing.md)）
...
> **实现推迟至 v2.0**。v0.1–v1.x 阶段仅提供 `kun` 脚本执行器和 `libkunlang.so` 共享库。
...
| 版本 | 变更 |
|------|------|
| 2026.07.16 | 单元测试系统重设计：`test` 子命令行更新——从 `<directory>`/`--filter=<pattern>`/`--verbose` 改为 `<module>_test.kun` 测试文件、`Test` 类型值、`--filter`/`--timeout`/`--parallel`/`--fail-fast`/`--report` 五个选项；脚本入口章节「`main` 与 `test*` 的 `handle with` 限制」更名为「`main` 与 `Test.body` 的 `handle with` 限制」，入口级上下文从 `main`/`test*` 改为 `main`/`Test.body`；详见 [单元测试设计](testing.md) |
| 2026.07.16 | 沙箱加固（参考 Z-Jail 分层防御思路）：安全参数新增 `--audit=<path>` JSON 审计记录选项（含脚本路径、安全参数、沙箱配置、退出码、脚本内容 SHA-256 哈希、时间戳，用于 CI/合规追溯）；审计记录的 `sandbox_layers` 字段反映实际安装的沙箱层（含新增的 dumpable/capabilities_dropped/fd_scrub） |
| 2026.07.16 | 三项设计调整：（1）零参效应函数约定——`test*` 函数识别规则同步更新（零参效应函数 `Unit ! {E}` / `TestResult ! {E}`）；Command 的 `?`/`!` 后缀糖已废弃，零参函数执行的 `!` 后缀是独立特性（2）守卫子句改用 `if`（3）类型标注与值绑定支持同行形式 `name : Type = expr` |
| 2026.07.15 | 代数效应与命令系统设计配套更新：新增 `--allow-ffi` 安全控制参数（运行时检查 FFI 效应到达 `main`，未启用则退出码 126）；脚本入口章节补充 `main` 与 `test*` 的 `handle with` 限制（仅入口函数可用）、`main` 边界与效应集校验；所有示例改用 `let in`（废弃 `do`/`do in`），`main` 签名添加效应集标注 |
| 2026.06.18 | Kun Shell 章节添加实现推迟至 v2.0 注记 |
