...
`.kun` 脚本通过 `main` 函数作为执行入口。`main` 与 `test*` 函数是入口级函数，允许使用 `handle with` 消解效应（业务函数不可使用，详见 [代数效应系统](syntax.md#代数效应系统)）。
...
### `main` 与 `test*` 的 `handle with` 限制

`handle with` 表达式**仅在 `main` 与 `test*` 函数内可用**——业务函数只声明效应不消解，效应冒泡到入口函数集中消解。

- **`main`**：程序入口，允许消解用户效应（内置效应自动注入默认 handler）
- **`test*`**：`test` 前缀命名的测试函数（如 `testFoo`、`testUserService`），与 `main` 同级，允许使用 `handle with`
