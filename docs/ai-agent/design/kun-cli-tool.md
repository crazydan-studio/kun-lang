...
| `test` | `<module>_test.kun` 测试文件、`TestCase` 类型值、`--filter`/`--timeout`/`--parallel`/`--fail-fast`/`--report`（推迟 v1.2） |
...
| 版本 | 变更 |
|------|------|
| 2026.07.16 | 测试类型重命名与 `Test` 模块化：`type Test = Test {...}` Record 重命名为 `type TestCase = TestCase {...}`（消除「类型与效应同名」歧义）；`Test` 名专用于效应（`! {Test, e}`）与模块（`Test.with`/`Test.timeout`/`Test.describe`，同名消歧）；新增 `test` 构造器与 `Test.with`/`Test.timeout`/`Test.describe` 链式 `|>` 调用；脚本入口章节「`main` 与 `Test` 类型 `body` 字段 的 `handle with` 限制」更名为「`main` 与 `TestCase.body` 的 `handle with` 限制」，入口级上下文表 `Test` 类型 `body` 字段 改为 `TestCase.body`；`test` 子命令行中 `Test` 类型值改为 `TestCase` 类型值；详见 [单元测试设计](testing.md) |
| 2026.07.16 | 单元测试系统重设计：`test` 子命令行更新——从 `<directory>`/`--filter=<pattern>`/`--verbose` 改为 `<module>_test.kun` 测试文件、`Test` 类型值、`--filter`/`--timeout`/`--parallel`/`--fail-fast`/`--report` 五个选项；脚本入口章节「`main` 与 `test*` 的 `handle with` 限制」更名为「`main` 与 `Test` 类型 `body` 字段 的 `handle with` 限制」，入口级上下文从 `main`/`test*` 改为 `main`/`Test` 类型 `body` 字段；详见 [单元测试设计](testing.md) |
