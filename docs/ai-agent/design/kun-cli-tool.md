...
| `test` | `<directory>`、`--filter=<pattern>`、`--verbose` | 发现并运行测试文件（`tests/test-*.kun`），输出测试结果摘要 |
...
| `--allow-exec <path>` | 允许子进程执行指定路径的可执行文件。可多次指定。未列出的可执行文件 fork/exec 被拒绝（返回 PermissionDenied） |
| `--allow-net` | 开放网络出站 |
| `--no-sandbox` | 完全关闭沙箱隔离 |
| `--force` | 跳过安全确认——当脚本的路径/网络使用声明与实际情况不一致时，不提示用户而直接运行。仅用于受信任脚本的自动化运行 |
| `--dry-run` | 安全预览模式——解析脚本并检查类型，但不执行任何副作用（不 fork 子进程，不读写文件，不修改环境变量）。输出将执行的操作列表 |
