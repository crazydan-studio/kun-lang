...
3. **原始输出的逃生口**：通过 `exec` 原语获取原始输出，命令函数不保证执行的命令与传参一一对应
4. **渐进增强**：无 CDF 的命令通过 `exec` 原语使用，功能受限但不会完全阻塞
5. **来源可信**：CDF 文件通过密码学签名建立信任链，防止恶意签名定义
...
```kun
git.commit : { all : Bool, amend : Bool, message : Maybe String } -> IO (Result Unit (List IOError))
git.push   : { force : Bool, verbose : Bool } -> Maybe String -> IO (Result Unit (List IOError))
```
...
命令函数的映射原则：只映射 Kun 无法原生获取、需要外部命令提供结构化结果的命令。文本变换类命令（`sed`、`awk`、`sort`、`uniq`、`cut`、`tr`、`head`、`tail`）和内容查看类命令（`cat`、`wc`）由 Kun 标准库和语言特性覆盖，不映射为命令函数。
...
| 查找检索 | `find`、`locate` |
| 进程信息 | `ps` |
| 系统信息 | `free`、`uname`、`lscpu`、`uptime` |
| 网络 | `curl`、`dig`、`ping` |
| 文件操作（仅退出码） | `cp`、`mv`、`rm`、`mkdir`、`chmod`、`chown`、`ln`、`readlink` |
...
基于 CDF 自动生成 Kun 函数签名。输出格式相关参数（`-l`、`-h`、`--format`、`--json` 等）**不在** CDF 中声明，运行时自动处理：
