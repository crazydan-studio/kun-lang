...
`Test` 是一个**标准库效应**（编译器保留名——用户不可定义同名 `effect`，但作为模块名仍可被用户的项目模块覆盖，详见 [类型系统 - 内置效应](type-system.md#内置效应)），提供三个测试操作：
...
- 实现机制：每个测试运行在独立的子进程（`fork`）中，超时后向子进程发送 `SIGTERM`→5s→`SIGKILL`→`waitpid` 回收，释放所有资源（包括子进程、临时文件、内存）。Kun 运行时是单线程的，`--parallel` 通过多进程（`fork`）实现并发，而非多线程
