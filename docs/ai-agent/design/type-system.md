...
- 创建时显式指定字段值覆盖缺省值
- 缺省值不影响 Record 的类型结构：`{ port : Int = 8080 }` 与 `{ port : Int }` 是同一类型。缺省值不同的两个类型在结构上等价
- 模式匹配和解构不受缺省值影响——解构取得的是实际值，非缺省值
- Record 创建后缺省值已展开为实际值，后续 update 操作（`{ record | field = val }`）不受缺省值影响
- 缺省值仅在创建时生效，运行时值无"来自缺省"标记

#### 设计约束

**约束 1：`{ }` 需要类型上下文**

空 Record 字面量 `{ }` 要求有足够的类型上下文可供推断：
...
-- 正确：函数参数提供类型信息
connect { port = 9090 }    -- connect 的参数类型 Config 提供了字段和缺省值信息

-- 错误：无类型上下文
x = { }
```

类型检查器应报告"无法推断 Record 类型，请添加类型标注"。

**约束 2：禁止自引用和交叉引用缺省值**

字段的缺省值不能引用同一 Record 中的其他字段：

```kun
-- 错误：自引用
{ x : Int = 5, y : Int = x }    -- y 引用了 x，但 x 可能在创建时被覆盖

-- 正确：所有字段独立指定缺省值
{ x : Int = 5, y : Int = 10 }
```

原因是缺省值在编译期展开，而字段被覆盖发生在创建时（运行时），编译期无法确定 `x` 的最终值。

**约束 3：禁止泛型字段的缺省值**

```kun
-- 错误：泛型字段无法绑定缺省值
{ value : a = ... }    -- a 未知，无法确定缺省值类型
```
...
```kun
-- 错误：缺省值类型与字段类型不匹配
{ count : Int = "hello" }    -- String 不是 Int
```

**约束 5：运行时表示不受缺省值影响**

缺省值纯粹是编译期语法糖。在运行时，通过缺省值创建的字段与显式指定的字段没有区别——不存储"是否来自缺省"标记，不影响 C ABI 兼容性。
