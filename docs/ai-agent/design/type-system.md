...
> **编译器内置**：效应跟踪通过编译器 AST 标记 + 类型签名中的 `!` 标注 + Primitive 函数效应标记（Phase 3+ `PrimitiveBinding.is_effect`）三层机制实现。不引入 `effect` 关键字。——MVP 阶段 Primitive 函数的效应性通过硬编码命名空间前缀匹配识别，Phase 3+ 迁移至编译期常量表。

### 效应函数自动推断

Kun 通过以下三层机制自动推断函数的效应性：

1. **`do` 块 AST 扫描**：含 `do` 块的函数自动标记为效应函数
2. **`(a -> b)!` 回调标注**：签名中声明了 `!` 参数的函数标记为效应函数
3. **Primitive 函数效应标记**：标准库 Primitive 函数的效应性由编译期常量表 `PrimitiveBinding.is_effect` 记录

> **MVP 过渡**（v0.1）：第 3 层因 Primitive 表尚未实现，暂时通过硬编码命名空间前缀匹配——以下命名空间的所有函数均为效应函数：`IO.*`、`File.*`、`Env.*`、`Process.*`、`Task.*`、`Random.*`；`Signal.on` 为效应函数（`Signal` 模块其余函数为纯函数）。`Cmd.<bin>` 构造 `Command` 值及 `Cmd` 装饰/组合/管道函数为纯操作，`Cmd.<bin>?`/`Cmd.<bin>!`、`Cmd.pipe?`/`Cmd.pipe!`、`Cmd.timeout`、`Cmd.retry`、`Cmd.execSafe`、`Cmd.which`、`Cmd.exec` 为效应函数。Phase 3+ 用 `PrimitiveBinding.is_effect: bool` 字段替代硬编码列表。

纯函数（无 `do` 块、无 `!` 参数声明）不能调用效应函数——编译期拒绝。`(a -> b)!` 参数使函数自身成为效应函数。函数名不添加效应标记，效应性由编译器推断、文档生成（`kun doc`）和 IDE/LSP 提供可见性。
...
- `List.map : (a -> b) -> List a -> List b` **不能**接收 `EffectFn` 类型的参数——因为 `a -> b` 是 `Fn(a, b)` 的语法糖，而类型变量 `a` 在此处期望与 `Fn(_, _)` 合一，与 `EffectFn(a, b)` 因结构等价不同而无法合一
...
> 注：含 `do` 块或调用效应函数的用户定义函数，编译器在约束生成阶段自动为其赋予 `EffectFn` 内部类型（而非 `Fn`）。效应函数的识别来源包括：(1) 函数体含 `do` 块，(2) 签名含 `!` 参数，(3) 调用 Primitive 表中标记为 `is_effect` 的函数（Phase 3+ 替代 MVP 的硬编码命名空间匹配）。用户定义的效应函数**可以**传入 `!` 标注的参数（二者同为 `EffectFn`），但**不能**传入普通高阶函数的非 `!` 回调参数（如 `List.map`，其参数类型为 `Fn(a, b)`，与 `EffectFn` 不兼容）。用户无需在函数签名中书写 `!`——`->` 语法保持不变，编译器根据函数体自动选择 `Fn` 或 `EffectFn` 内部表示。

需要接受效应回调的高阶函数（如 `List.iter`）必须显式标注 `(a -> b)!` 以使用 `EffectFn` 类型构造器——其参数位置接受 `EffectFn` 而非 `Fn`。不含 `!` 的函数位置（包括类型变量在函数位置的实例）统一解析为 `Fn`，`EffectFn` 无法在此位置出现。
...
- 识别 Primitive 表中 `is_effect = true` 的函数调用为效应调用（Phase 3+ 替代 MVP 硬编码命名空间匹配）

**纯函数约束：**
- 验证纯函数体中无效应函数调用
- 验证纯函数签名中无 `!` 参数声明
- 验证纯函数返回类型非 `Unit`
- 验证 `let in` 内无效应函数调用、定义和效应命名空间函数引用——包括作为值绑定的情况（如 `f = IO.println`），编译错误
...
- 验证 `do` 块外的代码无效应函数调用（Primitive 表中标记为 `is_effect` 的函数或含 `do`/`!` 的用户定义函数）
...
**隐式 `do` 上下文（unbound case/if 分支）：**
- 在 `do` 上下文中，未被值绑定、不作为函数返回值的 `case`/`if` 的各分支识别为隐式 `do` 块
- 隐式 `do` 分支内无效应函数调用时产生告警
- `defer` 在隐式 `do` 分支中的作用域为该分支自身的隐式 `do`

效应检查失败根据严重程度分为：
- **编译错误**（Error）：类型级别的不一致——拒绝编译，退出码 1
- **告警**（Warning）：潜在浪费或无意义代码——输出警告，编译通过

#### 告警清单

| 告警场景 | 说明 |
|---------|------|
| 顶级 `do`/`do in` 无效应调用 | `do` 是为效应操作设计的，无效应调用无意义 |
| Unbound 隐式 `do` 分支无效应调用 | 该分支内仅有纯操作，无效应目的 |
| `do` body 最后一条语句为纯表达式 | 纯表达式的结果不被使用则浪费 |
| 纯函数调用结果绑定到变量后未引用 | 计算结果未被使用，浪费 CPU |
| 纯函数调用结果绑定到 `_` | 等同于未引用结果，无意义 |
| 非效应表达式作为独立语句且结果未消费 | `+`、`-`、`if`、`case` 等在 `do`/`let` 内作为独立语句且结果未消费时告警 |
| 已绑定变量未被使用 | 变量绑定（效应或纯函数）后后续无引用 |

#### 错误清单（新增）

| 错误场景 | 说明 |
|---------|------|
| `let in` 内出现效应函数调用/定义/引用 | 纯性约束违反，编译错误 |
| `do`/`do in` 空 body | 无操作无意义，编译错误 |
| `let in` 空 body | 直接书写表达式即可，编译错误 |
| `do in` 的 `in` 内表达式结果为 `Unit` | `do in` 用于返回值，`Unit` 返回值无意义；使用裸 `do`，编译错误 |
| 纯函数内定义或调用效应函数 | 违反纯性约束，编译错误 |
| 同一 scope 内 `do`/`let` 互嵌套 | 效应上下文与纯上下文不可混用，编译错误 |
| 变量重复绑定 | Kun 为不可变语言，同 scope 内同一变量名不可二次绑定，编译错误 |

效应检查失败产生 `TypeError`，纳入统一的错误报告。

> **注意**：`let ... in` 绑定、`defer` 资源清理和 f-string 插值均有类型检查级语义：
> - `let ... in` 绑定表达式必须为纯（不得含任何效应操作的痕迹），绑定采用延迟求值
> - `do in` 形式在副作用执行后返回值——`in` 之后的表达式处于效应上下文，可调用效应函数。`in` 结果必须为非 `Unit` 类型
> - `defer expr` 仅能在 `do` 块内使用，类型为 `Unit`。`defer` 的作用域为最接近的外层 `do`（含隐式 `do`）
