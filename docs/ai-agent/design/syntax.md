...
> **`cmd` 选项 Record 使用非标准简写语义**——`{ a }` ≡ `{ a = true }`（**flag 简写**，用于表示布尔旗标）。标准 Record 字面量中 `{ a }` 是 punning（`{ a = a }`，引用同名绑定变量）。`cmd` 选项位置是特殊语法——解析时进入"选项简写模式"，简写字段强制解释为 `= true`；标准 Record 字面量在普通表达式位置，简写字段解释为 punning。两者形式相同但语义不同，这是 `cmd` 字面量为命令选项场景做的特殊处理，不污染标准 Record 语义。
