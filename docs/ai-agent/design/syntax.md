...
`DateTime` 类型采用 `%` 引导符进入格式模式，后接字段名组合的格式模板（如 `%yyyy-MM-dd`）。完整字段名列表见 [`DateTime` 模块](../design/standard-library.md#datetime)。
...
- `do` 块内使用 `=` 绑定值
- 效应函数（`Cmd.*`、`IO.*`、`File.*`、`Env.*`、`Process.*`、`Signal.*`、`Sys.*` 命名空间的函数 + 用户定义含 `do` 块的函数）只能在 `do` 块中调用
- 含 `do` 块的函数自动标记为效应函数
- 纯函数（无 `do` 块）不能调用效应函数
