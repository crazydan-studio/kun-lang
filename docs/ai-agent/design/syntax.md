...
1. **块表达式**：一切皆为表达式，每条表达式返回一个确定类型的值；`let in` 是块表达式，统一所有多语句形式
...
Kun 采用**块表达式**（Block Expression）范式——程序中所有构造均为表达式，求值后得到具体类型的值。类型的实例即为值。`let in` 是块表达式，统一所有多语句形式（**废弃 `do`/`do in`**）。

### 表达式分类

| 表达式形式 | 是否块表达式 | 结果类型 | 说明 |
|-----------|--------------|---------|------|
...
| `let <body> in <expr>` | 是 | `<expr>` 的类型 | 块表达式 |
| `let <body>`（省略 `in`） | 是 | `Unit` | ≡ `let <body> in ()` |
| `handle <expr> with <handler>` | 是 | `<expr>` 的类型（效应被消解） | 仅 `main`/`Test.body` 内可用 |
...
- **Unbound**（结果未被值绑定，也不作为函数返回值）：继承外层 `let in` 效应上下文，分支内可直接写多语句，结果视为 `Unit`
- **Bound/Returned**（结果被值绑定或作为函数返回值）：各分支必须返回相同类型的值。多语句分支必须用 `let in` 包裹为块表达式

### 函数体与 Scope 规则

- 每个函数体均为独立 scope，块表达式规则从头应用
- 函数体无论是单语句还是多语句，效应函数体使用 `let in` 包裹（返回 `Unit` 可省略 `in`）；纯函数体单表达式时可省略 `let in` 包裹
...
| Token 类别 | 示例 | 说明 |
|-----------|------|------|
| 关键字 | `type`、`alias`、`effect`、`handler`、`handle`、`with`、`extern`、`cmd`、`continue`、`abort`、`case`、`of`、`if`、`then`、`else`、`let`、`in`、`defer`、`import`、`export`、`as`、`not`、`true`、`false` | 详见关键字表。`Nil` 和 `Some` 不是关键字——它们是 `Nilable` ADT 的变体名，通过缺省导入可用（类似 `Ok`/`Err`）。`assert`/`fail`/`skip` 不是关键字——它们是 `Test` 标准库效应的操作（需 `import Test`） |
...
匿名函数为独立 scope，块表达式规则从头应用。函数体规则与具名函数一致：

- **效应匿名函数**：函数体使用 `let in` 包裹（返回 `Unit` 可省略 `in`）：
  ```kun
  \x ->
    let
    ...
    in
      ()
  ```
- **纯匿名函数**：函数体单表达式时可省略 `let in` 包裹；多语句时须用 `let in` 包裹：
  ```kun
...
`let in` 是 Kun 的块表达式，统一所有多语句形式。**废弃 `do`/`do in`**——所有原 `do`/`do in` 用 `let in` 替代。
...
**规则**：`let <body>`（无 `in`）≡ `let <body> in ()`，固定返回 `Unit`，效应集为体内效应并集。

### 解析器识别规则（缩进对齐）

`let` 后的语句块以**缩进对齐**判定结束：

1. `let` 后遇 `in` → 完整形式，`in` 后必须有表达式
2. `let` 后无 `in`，语句缩进对齐 → 省略形式，返回 Unit
3. `let` 后无 `in`，缩进回退 → 块结束
4. `let in`（空 body + 空 expr）→ 编译错误
5. 缩进不一致 → lint 告警
...
let
  IO.println "line1"
  IO.println "line2"
IO.println "outside"          // 缩进回退，let 块结束（省略 in）
...
- `in` 后无表达式 → 编译错误（若意图省略，不应写 `in`）
- `let` 单语句无 `in` → lint 告警"单语句 let 省略 in 可能是笔误，建议显式 `in ()`"

适用场景：`main` 函数、仅副作用的函数、循环体等 Unit 返回场景。

### 效应集推导

`let in` 效应集 = 体内所有效应语句的并集：
...
`case`/`if` 表达式中，当结果被值绑定或作为函数返回值（bound/returned）时，多语句分支须用 `let in` 包裹为块表达式。单表达式分支直接书写即可。各分支结果类型必须相同：
...
`do`/`do in` 不再是关键字，使用 `let in` 统一所有多语句形式。
...
`let in` 统一后，分支包裹规则按 unbound/bound 区分：

1. **Unbound 分支**（结果未被值绑定，不作为返回值）：继承外层 `let in` 效应上下文，分支内可直接写多语句，结果视为 `Unit`
2. **Bound 分支**（结果被值绑定或作为返回值）：多语句分支须用 `let in` 包裹

```kun
// unbound 分支（在 let in 块内，结果未消费）
let
...
- **Unbound**（结果未被消费，处于外层 `let in` 上下文）：分支内可直接书写多语句，结果视为 `Unit`，无需显式包裹
- **Bound/Returned**（结果被值绑定或作为函数返回值）：多语句分支须用 `let in` 包裹为块表达式；单表达式分支直接书写。各分支结果类型必须相同
...
`defer expr` 在 `let in` 块内注册资源清理操作。`defer` 绑定到**最近 `let in` 块**，块退出时（正常返回或 panic unwind）按 LIFO 逆序执行。
  ...
  case File.createTemp! of
    Ok tmp ->
      let
        ...
        // defer (File.remove tmp) 在此 let in 块退出时执行
      in
        ()
...
`defer` 作用域 = **所在 `let in` 块**，嵌套时各层独立管理：
...
1. `defer expr` 中 `expr` 在所在 `let in` 块退出时执行
2. 多个 `defer` 按 LIFO 逆序执行
3. panic 时仍执行 `defer`（unwind 语义）
4. `defer` 仅在 `let in` 块内有效（不可在纯表达式顶层）
5. `defer` 的 `expr` 效应并入所在 `let in` 块
...
每个函数体为独立 scope，块表达式规则从头应用：

- **效应函数**：函数体使用 `let in` 包裹（返回 `Unit` 可省略 `in`），无论单语句还是多语句：
  ```kun
  main : List String -> Unit ! {IO}
  main = \_ ->
    let
...
> Mock handler 用于 `Test.with` 字段消解用户效应（声明式效应隔离），详见 [单元测试设计 - Handler 组合实现效应隔离](testing.md#handler-组合实现效应隔离)。
...
`handle with` 表达式消解效应集，**仅在 `main` 函数与 `Test` 值的 `body` 字段内可用**：
...
|---|---|---|
| `main` | ✅ | 程序入口 |
| `Test.body` | ✅ | `Test` 类型值的 `body` 字段，由 `kun test` 运行器在入口级上下文执行（详见 [测试与 `Test` 效应](#测试与-test-效应)、[单元测试设计](testing.md)） |
| 其他业务函数 | ❌ | 只声明效应，不消解 |
...
业务函数声明效应 → 冒泡到调用者 → 最终到 `main`（或 `Test.body`）→ 入口级上下文内 `handle` 消解。
...
测试用例是 `<module>_test.kun` 文件中**导出的 `Test` 类型值**（而非 `test*` 前缀函数）。`assert`/`fail`/`skip` 是 `Test` 效应的操作，通过 `abort` 终止测试（不再使用 panic）。

> 完整设计（`Test` 类型 Record、`Test` 效应、`testHandler`、执行模型、并行隔离、生命周期、报告格式、`kun test` 命令选项）详见 [单元测试设计](testing.md)。本节仅描述语法层面要点。
...
- `Test` 是**标准库效应**（非保留名——与 `DB`/`Log` 等用户效应同构）；`testHandler : Handler {Test} TestResult ! {IO}` 是 `kun` 二进制内置 handler（运行器提供，与 IO/File 等内置效应默认 handler 同级）
...
- `Test.with` 字段：可选 handler，消解 `body` 的用户效应 `e`；多个用户效应通过 `>>` 组合为单一 handler；`Nil` 表示 `e` 必须为空或仅含内置效应（由运行时沙箱消解）
...
export (testFetchUser)   // ← 仅导出的 Test 值才会被运行
...
- 支持 `--filter`（glob 匹配 `Test.name`）/`--timeout`/`--parallel`/`--fail-fast`/`--report text|json` 选项
- 无 `beforeAll`/`afterAll`/`beforeEach`/`afterEach` 隐式全局钩子；Setup/teardown 通过 `defer`（在 `Test` 效应 `abort` 路径下也会执行）+ handler 组合显式表达

### 入口级 `handle with` 与 `Test.body`

`handle with` 表达式**仅在 `main` 函数与 `Test` 值的 `body` 字段内可用**（详见 [`handle with` 表达式](#handle-with-表达式)）。`kun test` 运行器在入口级上下文执行 `Test.body`（包装 → `Test.with` 消解用户效应 → `testHandler` 消解 `Test` 效应），因此 `body` 内可使用 `handle with`；用户效应通常通过 `Test.with` 字段声明式消解。
...
Stream 必须由终端操作消费——未被消费的 Stream 导致子进程变为僵尸和 fd 泄漏。**Stream 消费检查的作用域为单个 `let in` 块**：块内构造的 Stream 必须在本块内消费；跨块传递（作为函数参数/返回值/绑定到外层）视为已消费。详见 [类型系统 - Stream 消费检查](type-system.md#stream-消费检查)。
...
| 版本 | 变更 |
|------|------|
| 2026.07.16 | 单元测试系统重设计：「测试函数与 `assert`」章节更名为「测试与 `Test` 效应」——测试用例从 `test*` 前缀函数改为导出的 `Test` 类型值（`type Test = Test { name, description, timeout, body, with }`）；`assert`/`fail`/`skip` 从 panic 函数改为 `Test` 效应操作（`effect Test = { assert, fail, skip }`，通过 `abort` 终止，不再 panic）；新增 `testHandler` 运行器内置 handler；新增 `Test.with` 字段声明式效应隔离；`assert` 不再是关键字（移除出关键字表与 Token 表，改为 `Test` 效应的操作，需 `import Test`）；`handle with` 表达式入口级上下文从 `main`/`test*` 改为 `main`/`Test.body`（表格、识别机制、效应流向同步更新）；多语句作用域表 `handle with` 行更新；Mock handler 示例路径从 `test/` 改为 `lib/`；详见 [单元测试设计](testing.md) |
| 2026.07.16 | 三项设计调整：（1）零参效应函数约定——签名 `-> T ! {E}` / `Unit -> T ! {E}` 改为 `T ! {E}`（无 `->` 前缀），`effect`/`extern` 操作记录中 `Unit -> T` 改为 `T`，调用加 `!` 后缀（`Name!`），裸名 `Name` 为函数引用，`!` 后缀与已废弃的 Command 断言执行 `!` 是不同特性；测试函数签名改为 `Unit ! {E}` / `TestResult ! {E}`（2）守卫子句改用 `if`（移除 `when` 关键字，与 if 表达式复用同一关键字，上下文区分）（3）类型标注与值绑定支持同行形式 `name : Type = expr`，解析器同时支持同行与分两行；设计原则 #3 从"声明与实现分离"放宽为"可分离或同行" |
| 2026.07.16 | 新增「效应与模块同名」章节：效应名与模块名分属类型命名空间与值命名空间，同名合法；效应操作必须全名调用（`Cmd.exec`，不可裸名导入），模块纯函数可选择性导入裸名；效应操作不可被模块函数遮蔽；7 内置效应名为保留名与对应标准库模块同名 |
| 2026.07.15 | 代数效应与命令系统重设计：更名"单一表达式范式"→"块表达式"；统一 `let in`（废弃 `do`/`do in`，Unit 返回可省略 `in`，三种语句类型，缩进对齐结束判定，效应集推导）；新增 `effect`/`handler`/`handle with` 语法（限入口 `main`/`test*`）；新增 `continue`/`abort` 控制流原语（每分支二选一，不可作值传递）；新增 `extern` 块语法（FFI，仅 Linux，`FfiBuffer` 不逃逸，`Opaque` 幻影类型）；新增 `cmd` 字面量四段式；新增 `assert`/`TestResult`/`test*` 函数规则；新增文档注释规范（多行 `//` + Markdown + 交叉引用）；新增 `defer` 与 `let in` 关系（绑定最近块，LIFO，panic 时执行）；新增 `Int` 位运算与优先级（`shl`/`shr`/`ushr` > `&` > `^` > `\|`）；新增 unbound/bound 分支规则（继承外层效应上下文 / `let in` 包裹）；新增模块系统规则（默认私有、re-export、无 wildcard、别名解决冲突、选择性导入+全名引用、模块别名）；新增 `alias`/`type` 分离（结构 vs 名义等价，移除 `Newtype` 概念，不做 tag 擦除）；立即求值统一（移除 `let` 延迟求值，`Lazy`/`Stream` 显式惰性特区）；`|>` 退化为纯管道（不隐式触发 Command 执行）；移除 `?`/`!` 后缀；移除 `Cmd.<bin>`/`Cmd[ "..." ]`/`Cmd.withRawOpt`/`Cmd.pipe?`/`Cmd.pipe!` |
| 2026.06.19 | 单一表达式范式全面定稿（已废弃，新设计更名块表达式） |
| 2026.06.18 | Record/List rest 模式文档补全 |
| 2026.06.18 | Cmd API 精简（已废弃，新设计采用 `cmd` 字面量） |
| 2026.06.26 | Nil 去除字面量关键字地位 |
| 2026.06.18 | 审计修复：Stream 变换 API 补充 `filterMap` |
| 2026.06.18 | 审计修复：DateTime f-string `%` 引导符与 `DateTime.format` API 的差异化说明 |
| 2026.06.18 | 新增 Or 模式语法 |
| 2026.06.17 | 模块组织：明确项目模块必须置于 `lib/` 子目录 |
| 2026.06.15 | 审计修复三轮：`?.`/`??` 补入优先级层次图；关键字表格新增；`_` 参数/解构语义文档化 |
| 2026.06.15 | 审计修复：Nil 字面量补入字面量表格与 Nilable 章节 |
| 2026.06.14 | 函数类型语法新增 `(a -> b)!` 效应回调标注（已废弃，新设计采用效应集） |
| 2026.06.13 | 管道示例统一模块限定风格 |
| 2026.06.11 | 模块系统：目录即命名空间 |
