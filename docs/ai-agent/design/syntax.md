...
| 版本 | 变更 |
|------|------|
| 2026.07.16 | 测试类型重命名与 `Test` 模块化：`type Test = Test {...}` Record 重命名为 `type TestCase = TestCase {...}`（消除「类型与效应同名」歧义）；`Test` 名专用于效应（`! {Test, e}`）与模块（`Test.with`/`Test.timeout`/`Test.describe`，同名消歧）；新增 `test : String -> (Unit ! {Test, e}) -> TestCase` 便捷构造器与 `Test.with`/`Test.timeout`/`Test.describe` 三个纯函数链式 `|>` 调用；导入语句从 `import Test (Test, Test(..), assert, fail, skip)` 改为 `import Test (Test, TestCase, test, assert, fail, skip)`；所有示例从 `Test { name, body, with }` 字面量改为 `test "..." (\ -> ...) |> Test.with ... |> Test.timeout ...` 链式形式；字段引用 `Test` 类型 `body` 字段/`Test.with` 改为 `TestCase.body`（字段）/`Test.with`（模块函数）；`handle with` 入口级上下文表 `Test` 类型 `body` 字段 改为 `TestCase.body`；新增「`Test` 模块——`test` 构造器与链式函数」小节；`effect Test`/`testHandler`/`TestResult` 不变 |
| 2026.07.16 | 单元测试系统重设计：「测试函数与 `assert`」章节更名为「测试与 `Test` 效应」——测试用例从 `test*` 前缀函数改为导出的 `Test` 类型值（`type Test = Test { name, description, timeout, body, with }`）；`assert`/`fail`/`skip` 从 panic 函数改为 `Test` 效应操作（`effect Test = { assert, fail, skip }`，通过 `abort` 终止，不再 panic）；新增 `testHandler` 运行器内置 handler；新增 `Test.with` 字段声明式效应隔离；`assert` 不再是关键字（移除出关键字表与 Token 表，改为 `Test` 效应的操作，需 `import Test`）；`handle with` 表达式入口级上下文从 `main`/`test*` 改为 `main`/`Test` 类型 `body` 字段（表格、识别机制、效应流向同步更新）；多语句作用域表 `handle with` 行更新；Mock handler 示例路径从 `test/` 改为 `lib/`；详见 [单元测试设计](testing.md) |
