...
| `handle <expr> with <handler>` | 是 | `<expr>` 的类型（效应被消解） | 仅 `main`/`test*` 内可用 |
...
| Token 类别 | 示例 | 说明 |
|-----------|------|------|
| 关键字 | `type`、`alias`、`effect`、`handler`、`handle`、`with`、`extern`、`cmd`、`continue`、`abort`、`assert`、`case`、`of`、`if`、`then`、`else`、`let`、`in`、`defer`、`import`、`export`、`as`、`not`、`true`、`false` | 详见关键字表。`Nil` 和 `Some` 不是关键字——它们是 `Nilable` ADT 的变体名，通过缺省导入可用（类似 `Ok`/`Err`） |
...
`handle with` 表达式消解效应集，**仅在 `main` 与 `test*` 函数内可用**：
...
业务函数声明效应 → 冒泡到调用者 → 最终到 `main`/`test*` → 入口函数内 `handle` 消解。
