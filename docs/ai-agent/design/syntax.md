...
| Token 类别 | 示例 | 说明 |
|-----------|------|------|
| 关键字 | `type`、`alias`、`opaque`、`effect`、`handler`、`with`、`extern`、`cmd`、`continue`、`abort`、`case`、`of`、`if`、`then`、`else`、`let`、`in`、`do`、`defer`、`import`、`export`、`as`、`not`、`true`、`false` | 详见关键字表。`Nil` 和 `Some` 不是关键字——它们是 `Nilable` ADT 的变体名，通过缺省导入可用（类似 `Ok`/`Err`）。`assert`/`fail`/`skip` 不是关键字——它们是 `Test` 标准库效应的操作（需 `import Test`） |
...
`do`/`let` 后的语句块以**关键字定界**判定结束，**不依赖缩进量，依赖换行与行首 token**：
...
5. `with <handler>` 是 `do`/`let in` 块的**可选后缀**——`do <body> with <h>` 或 `let <body> in <expr> with <h>`，将 handler 绑定到整个前置块（仅 `main`/`TestCase.body` 入口级上下文可用，详见 [`do ... with` / `let ... in ... with` 表达式](#do--with--let--in--with-表达式)）
6. 缩进仅用于格式可读性，不影响解析；缩进不一致 → lint 告警
...
语句以**换行**分隔——每条语句独占一行。多行续行通过 `|>` 链或括号实现：

- `|>` 链可跨行：`cmd ... |> Cmd.exec` 可写成两行，`|>` 在行首标识续行
- 函数应用可跨行：若参数在括号内，跨行无歧义
- 无续行运算符的跨行函数应用需用括号包裹
...
终端操作签名含 `! e`——Stream 来源的效应（如 `Cmd.stream` 的 `Cmd` 效应）在终端消费时触发并传播。纯 Stream（`Stream.range`/`Stream.fromList`）的终端操作 `e` 实例化为 `! {}`（纯），可在纯上下文使用。详见 [标准库 - Stream 终端操作的效应传播](standard-library.md#stream)。
