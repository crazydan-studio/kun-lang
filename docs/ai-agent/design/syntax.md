...
7. **显式执行**：Command 执行全显式（`Cmd.exec`/`Cmd.execSafe`/`Cmd.streamLines`/`Cmd.streamBytes`），无 Command 的 `?`/`!` 后缀糖，无 `|>` 隐式触发；零参函数执行用 `!` 后缀（见[类型系统 - 零参效应函数类型](type-system.md#零参效应函数类型-t-e)）
...
| `do <body> with <handler>` / `let <body> in <expr> with <handler>` | 是 | `<body>`/`<expr>` 的类型（效应被消解） | 仅 `main`/`TestCase.body` 内可用；`with` 绑定 handler 到整个前置 `do`/`let in` 块 |
...
| Token 类别 | 示例 | 说明 |
|-----------|------|------|
| 关键字 | `type`、`alias`、`opaque`、`effect`、`handler`、`with`、`extern`、`cmd`、`continue`、`abort`、`case`、`of`、`if`、`then`、`else`、`let`、`in`、`do`、`defer`、`import`、`export`、`as`、`not`、`shl`、`shr`、`ushr`、`true`、`false` | 详见关键字表。`Nil` 和 `Some` 不是关键字——它们是 `Nilable` ADT 的变体名，通过缺省导入可用（类似 `Ok`/`Err`）。`assert`/`fail`/`skip` 不是关键字——它们是 `Test` 标准库效应的操作（需 `import Test`）。`shl`/`shr`/`ushr` 是关键字（中缀位运算符），非标识符——`a shl 2` 是位运算，非函数应用 |
| 标识符 | `myVar`、`MyType`、`snake_case_func` | 小写开头为变量/函数，大写开头为类型/变体/模块 |
| 整数字面量 | `42`、`0xFF`、`0o77`、`0b1010`、`1_000_000` | 十进制/十六进制/八进制/二进制，`_` 分隔符可选。`0x` 前缀**始终产生 Int 字面量**（`0xFF` = Int 255），不产生 Bytes；Bytes 字面量使用 `0x"..."` 语法 |
| Bytes 字面量 | `0x"48656C6C6F"` | `0x"` 前缀 + 十六进制字符串 + `"` 闭合；产生 `Bytes` 类型，与 `0x` Int 字面量明确区分 |
...
| Bytes | `0x"..."` 前缀 + 十六进制字符串 + `"` 闭合 | `0x"48656C6C6F"`（= "Hello" 的字节），`0x"00FF"` |
| Char | 单引号包裹 | `'A'`, `'\n'`, `'好'` |
...
> **`0x` 前缀消歧**：`0x` 前缀始终产生 **Int** 字面量（`0xFF` = Int 255）。Bytes 字面量使用 `0x"..."` 语法（`0x"48656C6C6F"`），与 Int 明确区分。词法分析器在遇到 `0x` 后查看下一字符——若是数字/字母（即 hex digit），产生 Int 字面量；若是 `"`，产生 Bytes 字面量。**不可依赖类型上下文消歧**——词法阶段即可判定 token 类型，parser 无需类型信息。
...
> **多行字符串缩进敏感性**：多行字符串的"公共缩进基准"（determining common indentation）是 **lexing 阶段**规则——lexer 在扫描 `"""` 后跟踪每行前导空格，确定公共缩进并在生成 token 时剥离。这与"缩进不影响 parsing"的全局原则**不冲突**——后者约束 parser 的语法分析阶段，前者是 lexer 的字符处理规则（类似字符串内 `\n` 转义在 lexer 阶段处理）。两者职责分离：lexer 处理字符级规则（转义、公共缩进），parser 处理结构级规则（块边界、表达式归属）。
...
| 类别 | 关键字 |
|------|-------|
| 类型声明 | `type`、`alias`、`opaque` |
| 效应声明 | `effect`、`handler`、`with`、`extern` |
| 命令字面量 | `cmd` |
...
>
> `with` 是关键字，作为 `do <body> with <handler>` / `let <body> in <expr> with <handler>` 的可选后缀，将 handler 绑定到整个前置 `do`/`let in` 块（详见 [`do ... with` / `let ... in ... with` 表达式](#do--with--let--in--with-表达式)）。仅在 `main`/`TestCase.body` 入口级上下文可用。
>
> `handle` 关键字已移除（2026.07.18）——效应消解统一通过 `do...with` / `let...in...with` 表达，详见 [讨论记录](../discussions/discussion-handle-with-scope-and-syntax.md)。
...
`opaque type` 声明编译器内置的不透明类型——内部结构不暴露给用户，仅通过模块函数/Primitive 操作。用户**不可定义** `opaque type`（仅编译器内置）。

```kun
opaque type <Name>
```

- `opaque type` 无变体、无字段、无构造器——用户无法直接构造其值
- 值由 Primitive 函数或编译器内部产生（如 `test` Primitive 构造 `TestCase`）
- 与 `type` ADT 的区别：`type` 有变体/构造器（用户可构造/解构）；`opaque type` 无变体（用户不可构造/解构，仅通过 API 操作）
- 内置 `opaque type` 示例：`TestCase`（由 `test` Primitive 构造）、`Command`（注：`Command` 实为 `type` ADT，有 `Simple`/`Pipe` 变体，不是 `opaque type`）

> **`TestCase` 与 `Command` 的区别**：`TestCase` 是 `opaque type`（无变体，由 Primitive 构造）；`Command` 是 `type` ADT（有 `Simple`/`Pipe` 变体，由 `cmd` 字面量构造）。二者性质不同，不应混为一谈。
...
`do`/`let` 后的语句块以**关键字定界**判定结束。解析**不依赖缩进量**，但**换行**是语句分隔符，**行首 token** 用于续行判定：

1. `let <body> in <expr>` — `let` 后遇 `in` 关键字结束 body，`in` 后必须有表达式
2. `do <body>` — `do` 后语句持续至遇到定界符（详见下文「`do` 块终止条件」）
3. `let` 后**必须**有 `in`——不再支持无 `in` 的 `let <body>` 形式（Unit 返回用 `do`）
4. `let in`（空 body + 空 expr）或 `do`（空 body）→ 编译错误
5. `with <handler>` 是 `do`/`let in` 块的**可选后缀**——`do <body> with <h>` 或 `let <body> in <expr> with <h>`，将 handler 绑定到整个前置块（仅 `main`/`TestCase.body` 入口级上下文可用，详见 [`do ... with` / `let ... in ... with` 表达式](#do--with--let--in--with-表达式)）。`with` 作为 `do`/`let in` 的可选后缀关键字——行首 `with` 终止 `do` body，将其作为 handler 绑定
6. `do`/`let in` 块内语句以**换行**分隔；缩进仅用于格式可读性，不影响解析；缩进不一致 → lint 告警
...
> **注**：解析器不依赖缩进量来解析 `do`/`let` 块结构——所有代码块由显式关键字界定（`do`、`let...in`、`case...of`、`do...with`、`let...in...with`），依赖换行与行首 token 定界。分支体内多语句的边界通过 `pattern ->` / `else if` / `else` 关键字定界 + `case...of` 配对跟踪实现。缩进规则仅约束代码**格式**（可读性），不约束代码**语义**。
...
`do <body>` 的终止条件 = 遇到以下 token 之一：

1. `with`（handler 绑定后缀）—— `do` body 终止，`with` 后接 handler 表达式
2. `else` / `else if`（if 分支终止）—— `do` 在 `if` 分支体内时，遇到 `else` 终止
3. 下一个 `pattern ->`（case 分支终止，**贪婪归属内层 case**）—— `do` 在 `case` 分支体内时，遇到新的 `pattern ->` 终止；嵌套 `case` 时归属最内层未闭合的 case
4. `}` / `)`（括号闭合）—— `do` 在括号内时，遇到匹配的闭合括号终止
5. 文件结束（EOF）—— 顶层 `do`（如 `main = \_ -> do ...`）持续至文件结束
6. 换行 + 缩进回退到外层级别（`do` 所在函数/分支的缩进级别）—— 当 `do` 在嵌套上下文中（如 `let in` 块内）时，缩进回退到外层视为 `do` 块结束

> **`pattern ->` 歧义处理**：`Foo bar` 在表达式位置是函数应用、在 pattern 位置是构造器匹配，token 序列相同。解析器在 `do` body 内遇到行首的 `Constructor ident` 形式时，**优先尝试解析为 `pattern ->`**（case 分支终止信号）；若该 token 序列出现在 `do` body 表达式位置（如 `Foo bar` 作为函数调用），需用括号包裹以消除歧义：`let result = (Foo bar) in ...`。这避免了"无类型信息无法识别 pattern"的问题——解析器假设行首 `Constructor ident` 是 case pattern，强制要求表达式位置用括号。

### 语句分隔与多行续行

语句以**换行**分隔——每条语句独占一行。多行续行通过运算符或括号实现：

- **行首运算符续行**：`|>`/`<<`/`>>`/`<|`/`++`/`&&`/`||`/`+`/`-`/`*`/`/`/`&`/`|`/`^` 等运算符在行首出现时，标识续行（前一行的表达式延续到本行）。如 `cmd ... |> Cmd.exec` 可写成两行，`|>` 在行首
- **行尾运算符续行**：运算符在行尾出现时，下一行视为续行——`x +\n y` 合法（`+` 在行尾）
- **括号内续行**：函数应用参数在括号内（`()`/`[]`/`{}`）可跨行无歧义
- **裸函数应用跨行**：无续行运算符的跨行函数应用需用括号包裹或运算符续行——`List.map` 后直接换行下一行是 `(\x -> ...)` 不合法（解析器无法判定续行），需用 `|>` 链或括号包裹整个调用
- `do`/`let in` 块内语句以换行分隔，缩进仅用于可读性（解析依赖换行 + 行首 token）
...
// ❌ 不合法：裸函数应用跨行（解析器无法判定续行）
result = List.map
  (\x -> x + 1) [ 1, 2, 3 ]
// 应改为：result = (List.map (\x -> x + 1) [ 1, 2, 3 ])
// 或用 |> 链：[ 1, 2, 3 ] |> List.map (\x -> x + 1)
...
> **守卫条件的效应归属**：`case x of Pat if cond -> body` 中 `cond` 的效应并入**分支体**的效应集。守卫条件可为效应函数（如 `if File.exists path`），其效应传播到分支体。即：分支的整体效应集 = `cond` 效应集 ∪ `body` 效应集。这条规则确保分支体不会"绕过"守卫条件的效应安全检查——若 `cond` 调用 `File.exists`，分支整体必须标 `! {File}`，即使 `body` 本身是纯的。
...
> **嵌套 `case` 的贪婪终止**：内层 `case...of` 贪婪吞并所有后续 `pattern ->` 分支，直到遇到外层上下文终止信号（`else`/`else if`/`}`/`)`/换行缩进回退到外层 `case` 的层级/文件结束）。若外层 `case` 需要在内层 `case` 之后继续添加分支，必须用括号包裹内层 `case`：
> ```kun
> // ❌ 歧义：D -> 3 被贪婪归入内层 case
> case x of
>   A -> case y of B -> 1; C -> 2
...
>   D -> 3   // ← 属于外层 case
> ```
> 括号是唯一可靠的嵌套 `case` 边界标记——不依赖缩进量，依赖括号配对与换行。
...
//       cmd ... |> Cmd.streamLines |> Stream.filter ...
```
...
handler 实现在编译器源码（Rust）中，签名与实现彻底分离。
...
// continue 传不同参数：变换查询（handler 体内仅 continue 调用，optimizeQuery 是纯函数 → 产出 ! {}）
...
效应消解通过 `do`/`let in` 块的可选 `with` 后缀实现——`with <handler>` 绑定 handler 到整个前置 `do`/`let in` 块。**仅在 `main` 函数与 `TestCase` 值的 `body` 字段内可用**。

`handle` 关键字已移除（2026.07.18），所有 handler 绑定统一通过 `do...with` / `let...in...with` 表达（详见 [讨论记录](../discussions/discussion-handle-with-scope-and-syntax.md)）。

两种形式：

```kun
// Unit 返回：do <body> 块 + with <handler> 后缀
do
  <body>
with
  <handler>

// 值返回：let <body> in <expr> 块 + with <handler> 后缀
let
  <body>
in
  <expr>
...
- 块内效应集若被 handler 消解，则从外层效应集中移除；handler 自身产生的效应（`handlerEffects`）冒泡到外层
- 嵌套 `do...with` / `let...in...with` 编译错误（入口级限制下不存在嵌套场景）
...
业务函数声明效应 → 冒泡到调用者 → 最终到 `main`（或 `TestCase.body`）→ 入口级上下文内 `do...with` / `let...in...with` 消解。
...
with
  postgreHandler >> journaldLog
// 外层 do 块绑定 with；内层 case 分支的 do 不带 with（不消解效应）
```
...
- `Test` 是**标准库效应**（编译器保留名——用户不可定义同名 `effect`，但作为模块名仍可被用户的项目模块覆盖，详见 [类型系统 - 内置效应](type-system.md#内置效应)）也是**标准库模块**（`test`/`Test.with`/`Test.timeout`/`Test.describe`，同名消歧）；`testHandler : Handler {Test} TestResult ! {IO}` 是 `kun` 二进制内置 handler（运行器提供，与 IO/File 等内置效应默认 handler 同级）；`TestCase` 是编译器内置**不透明类型**（类似 `Command`），其 `body` 字段的效应变量 `e` 被存在性量化，由 `test` 构造器在构造时捕获
...
### 入口级 `do ... with` / `let ... in ... with` 与 `TestCase.body`

`do...with` / `let...in...with` 表达式**仅在 `main` 函数与 `TestCase` 值的 `body` 字段内可用**（详见 [`do ... with` / `let ... in ... with` 表达式](#do--with--let--in--with-表达式)）。`kun test` 运行器在入口级上下文执行 `TestCase.body`（包装 → `TestCase.with` 消解用户效应 → `testHandler` 消解 `Test` 效应），因此 `body` 内可使用 `do...with` / `let...in...with`；用户效应通常通过 `Test.with` 模块函数声明式消解。
...
> **`cmd` 选项 Record 的非标准简写语义**（与标准 Record 区分）：在 `cmd` 字面量的选项 Record 中，**简写 `{ a }` 等价于 `{ a = true }`**（用于表示布尔旗标），这是 `cmd` 选项 Record 的非标准语义。**标准 Record 字面量中 `{ a }` 是 punning**（等价于 `{ a = a }`，引用同名绑定变量）。两者形式相同但语义不同——`cmd` 选项 Record 在解析时进入"选项简写模式"，简写字段强制解释为 `= true`；标准 Record 字面量在普通表达式位置，简写字段解释为 punning。这是 `cmd` 字面量为命令选项场景做的特殊处理，不污染标准 Record 语义。
...
- **`!` 后缀执行**：优先级高于函数应用。`f a b!` → `f a (b!)`——`!` 绑定最近的零参函数引用。`DateTime.now!` 执行 `DateTime.now`；`(f x)!` 需括号显式分组。此规则与 [类型系统 - 零参效应函数类型](type-system.md#零参效应函数类型-t-e) 中"`!` 后缀优先级高于函数应用"的描述一致。
- **函数应用** `f a b` 优先级高于所有运算符（除 `!` 后缀外）：`f a b + c` → `(f a b) + c`；`f a |> g` → `(f a) |> g`
...
|---|---|---|
| 效应名（类型层） | 函数效应集 `! {Cmd}`、`Handler {Cmd} a`、`do ... with h`、`effect Cmd = {...}` | 类型命名空间 |
...
| 调用 | 归属 | 判定 |
|---|---|---|
| `Cmd.exec`/`Cmd.execSafe`/`Cmd.streamLines`/`Cmd.streamBytes`/`Cmd.which` | **效应操作** | 签名在 `effect Cmd = {...}` 中声明，调用产生 `! {Cmd}` |
...
3. **效应操作不可被模块函数遮蔽**：`effect Cmd` 声明后，同模块内不可再定义同名 `exec`/`execSafe`/`streamLines`/`streamBytes`/`which` 绑定（编译错误，避免 `Cmd.exec` 歧义）。
...
**视觉区分**：带 `EffectName.` 前缀且名为该效应声明的操作（如 `Cmd.exec`/`Cmd.execSafe`/`Cmd.streamLines`/`Cmd.streamBytes`/`Cmd.which`）的是效应操作；其余 `EffectName.<xxx>` 或选择性导入裸名的是模块函数。
...
> **保留名**：10 个效应名为编译器保留名——7 个内置效应（`IO`/`File`/`Cmd`/`Random`/`DateTime`/`Signal`/`FFI`）+ 3 个标准库效应（`Env`/`Process`/`Test`），用户不可定义同名 `effect`。保留名仅约束 `effect` 声明——模块名仍可被用户的项目模块覆盖（如用户可定义自己的 `Log` 模块）。详见 [类型系统 - 内置效应](type-system.md#内置效应)。
...
导入有三种可组合风格（同一 `import` 语句中可同时使用模块别名、选择性导入、重命名等特性，不互斥）：
...
```kun
Stream.toList  : Stream a -> List a ! e                       // 终端（效应多态）
Stream.iter    : (a -> Unit ! e) -> Stream a -> Unit ! e       // 终端
Stream.fold    : (b -> a -> b ! e) -> b -> Stream a -> b ! e   // 终端
Stream.string  : Stream String -> String ! e                   // 终端：全文收集（效应多态）
Stream.bytes   : Stream a -> Bytes ! e                         // 终端：二进制读取（效应多态）
Stream.find    : (a -> Bool) -> Stream a -> ?a ! e             // 终端（效应多态）
Stream.all     : (a -> Bool) -> Stream a -> Bool ! e           // 终端（效应多态）
Stream.any     : (a -> Bool) -> Stream a -> Bool ! e           // 终端（效应多态）
Stream.nth     : Int -> Stream a -> ?a ! e                     // 终端（效应多态）
```

终端操作签名含 `! e`——Stream 来源的效应（如 `Cmd.streamLines` 的 `Cmd` 效应）在终端消费时触发并传播。纯 Stream（`Stream.range`/`Stream.fromList`）的终端操作 `e` 实例化为 `! {}`（纯），可在纯上下文使用。详见 [标准库 - Stream 终端操作的效应传播](standard-library.md#stream)。
...
> **`.` 后跟关键字**：点号 `.` 后的关键字（如 `Test.with`、`TestCase.with`）作为模块/记录成员名，不触发关键字语义。解析器在 `.` 后进入成员访问上下文，关键字 token 在此上下文中作为标识符处理。
