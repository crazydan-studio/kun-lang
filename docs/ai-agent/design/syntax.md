...
7. **显式执行**：Command 执行全显式（`Cmd.exec`/`Cmd.execSafe`/`Cmd.stream`），无 `?`/`!` 后缀糖，无 `|>` 隐式触发
...
| Token 类别 | 示例 | 说明 |
|-----------|------|------|
| 关键字 | `type`、`alias`、`effect`、`handler`、`handle`、`with`、`extern`、`cmd`、`continue`、`abort`、`assert`、`case`、`of`、`if`、`then`、`else`、`let`、`in`、`defer`、`import`、`export`、`as`、`when`、`not`、`true`、`false` | 详见关键字表。`Nil` 和 `Some` 不是关键字——它们是 `Nilable` ADT 的变体名，通过缺省导入可用（类似 `Ok`/`Err`） |
...
| 命令字面量 | `cmd` |
| 控制流 | `if`、`then`、`else`、`case`、`of`、`when`、`continue`、`abort` |
| 绑定 | `let`、`in` |
| 清理 | `defer` |
| 测试 | `assert` |
| 模块 | `export`、`import`、`as` |
...
handler DB of
  query q ->
    let
      f = \_ -> continue (DB.query q)   // 编译错误：continue 不可在 lambda 中
    in
      f ()
...
7. **`!` 在函数类型中的角色**：`! {E}` 引导效应集；`! e` 效应多态变量；无 `!` ≡ `! {}`
