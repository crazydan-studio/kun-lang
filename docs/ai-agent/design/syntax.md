...
`in` 和绑定表达式中可使用 `case`/`if` 的多语句分支体（纯上下文，遵守纯性约束）：
  ...
  in
    case items of
      ...
      [x] ->
        x * 2
      list ->
        list |> List.sum |> \s -> s / List.length list
```

`let` 绑定采用**延迟求值（lazy evaluation）**——绑定时的表达式不立即计算，仅构建 thunk（闭包 + 求值函数）。`in` 表达式中首次引用某个绑定时才真正求值，结果被 memoize 缓存，后续引用直接返回缓存值。
...
若在 `case`/`if` 多语句分支中尝试等价写法，`=` 绑定按声明顺序求值——`isEven` 求值时 `isOdd` 尚未绑定，编译期或运行时报错。
...
| 效应调用 | 禁止 | 继承外层（`do` 块内可调用效应函数） |

多绑定的求值顺序：绑定之间按声明顺序求值（前面的绑定在后继绑定的表达式中可见）。thunk 的依赖拓扑求解由运行时按首次引用顺序自动完成——对用户透明。

> **纯性约束**：`let ... in` 绑定的表达式必须是纯的——不得包含 `do` 块、效应命名空间调用（`IO.*`、`File.*`、`Env.*`、`Process.*`、`Task.*`、`Random.*`、`Signal.on`）及 `Cmd.<bin>?`/`Cmd.<bin>!`/`Cmd.pipe?`/`Cmd.pipe!`/`Cmd.timeout`/`Cmd.retry`/`Cmd.which`/`Cmd.exec`/`Cmd.execSafe`。`let` 绑定采用延迟求值（仅在 `in` 之后被引用时才求值），若允许效应代码，效应执行时机不确定且可能被多次触发（泛化后多次引用）。效应代码必须在 `do` 块内使用 `=` 绑定顺序执行。
...
规则：
- `pattern ->` 后若为换行，解析器进入分支体多语句模式，后续语句以换行分隔
- **最后一条语句**的类型和值为整个分支的结果
- 分支边界由下一个 `pattern ->` 定界——解析器在 `pattern ->` 标记处开始新分支，不依赖缩进
- 分支体内的 `case...of` / `if` 通过配对跟踪确定嵌套层级——`pattern ->` 定界作用于**最近外层 `case`** 的分支层级，嵌套 `case` 的 `pattern ->` 不触发外层分支终止
- 分支体**继承外层上下文**：外层为 `do` 块时可直接调用效应函数；纯上下文中仅允许纯表达式和 `=` 绑定（按声明顺序求值，非惰性）
- 若分支体需独立效应上下文（如独立的 `defer` 生命周期），使用内嵌 `do` 块
...
`if` 是表达式，必有返回值。`else` 分支**可省略**——省略时隐式类型为 `Unit`，适用于仅需副作用的 guard 子句或提前退出场景。`else if then` 链可用于处理多分支。

`if` 分支体同 `case` 分支支持多语句序列——`then`/`else` 后若为换行，后续以换行分隔的语句属当前分支，**末语句为分支结果**。分支边界由 `else if`、`else` 或 `if` 表达式结束定界。
...
> 注：上例 `do` 块类型为 `Unit`，因为最后一条语句（`case` 的每个分支末尾）为 `IO.print`/`IO.println`，均返回 `Unit`。多语句分支体的末语句同样决定分支类型，进而决定 `do` 块类型。`do` 块类型由最后一条语句决定，并非固定为 `Unit`。
