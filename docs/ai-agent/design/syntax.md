...
| Token 类别 | 示例 | 说明 |
|-----------|------|------|
| 关键字 | `type`、`case`、`of`、`if`、`then`、`else`、`do`、`in`、`let`、`defer`、`import`、`export`、`as`、`when`、`not`、`true`、`false` | 详见关键字表。`Nil` 和 `Some` 不是关键字——它们是 `Nilable` ADT 的变体名，通过缺省导入可用（类似 `Ok`/`Err`） |
...
需要从效应上下文中返回值时使用 `do in`。`in` 之后的表达式结果即为整个 `do in` 的结果。`in` 处于效应上下文内，可调用效应函数，但 **不可嵌套 `let in`**——`in` 部分与 `do` body 属于同一函数 scope，适用 `do`/`let` 互斥规则。`do in` 的结果必须为非 `Unit` 类型；`in` 内表达式的结果为 `Unit` 时编译错误。
...
- `do in` 的 `in` 部分与 `do` body 属于同一函数 scope，同样不可嵌套 `let in`。bound case 分支的多语句须用 `do in`（效应上下文）或 `let in`（纯上下文）包裹
...
| 表达式分组 | `(expr)` | — | |
| 成员访问 | `.` | 左结合 | |
| 可选链 | `?.` | 左结合 | 语法糖，语法分析阶段脱糖为 `case e of Some x -> f x; Nil -> Nil` |
| 函数应用 | (空格) | 左结合 | |
| Nil 合并 | `??` | 右结合 | 语法糖，语法分析阶段脱糖为 `case e of Some x -> x; Nil -> d` |
| 一元 | `-`, `not` | 右结合 |
| 乘除 | `*`, `/`, `%` | 左结合 |
| 加减 | `+`, `-` | 左结合 |
...
4. **`?` 在类型中的角色**：`?T` 为 `Nilable T` 的语法糖（编译器内置 ADT `Nilable a = Some a | Nil`），`?.` 为可选链（语法分析阶段脱糖为 `case`），`??` 为 Nil 合并（语法分析阶段脱糖为 `case`），`? :` 为三元
...
| 2026.06.18 | Cmd API 精简：`Cmd.execSafe` 签名从 `Result Unit` 改为 `Command -> Result (Stream String) CommandError`；移除 `Cmd.stdoutToString`、`Cmd.stderrToString`；新增 `Cmd.<bin>!`/`Cmd.pipe!` |
| 2026.06.26 | Nil 去除字面量关键字地位——`Nil` 和 `Some` 同为 `Nilable` ADT 的缺省可用变体，非关键字；关键字表、字面量表、运算符说明同步更新 |
