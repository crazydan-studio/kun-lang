...
| DateTime | ✅ 设计定型 | 内置 `DateTime` 效应；`format` 返回 `Result String String` |
| Uid / Gid | ✅ 设计定型 | 用户/组 ID 数字类型（`type UserId = UserId Int` 单变体 ADT） |
| Decimal | ✅ 设计定型 | 精确十进制数值（非编译器内置） |
| TestResult | ✅ 设计定型 | `type TestResult = Pass \| Fail String \| Skip String`（仅由 `testHandler` 产出） |
...
| Hash | ✅ 设计定型 | SHA-256 哈希（`sha256`/`sha256Hex`；`md5`） |
| Base64 | ✅ 设计定型 | Base64 编解码（`encode`/`decode`） |
| Cli | ✅ 设计定型 | 类型驱动 CLI 解析，对标 argparse；auto --help；子命令/互斥组/透传 |
| Validator | ✅ 设计定型 | `oneOf`/`range`/`nonEmpty`/`regex`，供 `Cli.withValidator` 使用（依赖 Regex） |
...
| Kun Shell | ✅ 设计定型 | 交互式环境；详见 [Kun Shell](kun-shell.md) |
...
| 功能 | 状态 | 说明 |
|------|------|------|
| 单表达式语法 | ✅ 设计定型 | 多语句返回值用 `let in`，返回 `Unit` 用 `do`（≈ `let <body> in ()`） |
...
| 功能 | 状态 | 替代 |
|------|------|------|
| `do` / `do in` 块（旧形式） | ✅ 重设为语法糖 | `do <body>` 重设为 `let <body> in ()` 的语法糖（返回 `Unit`）；`do in` 不再使用，返回值用 `let <body> in <expr>` |
