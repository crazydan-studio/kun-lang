...
| 分级可用性模型 | ✅ 设计定型 | T1 内建 → T2 CDF → T3 自动推断 → T4 CDF-less 受限模式，CDF 是最佳路径但不是唯一路径 |
...
| 命令可用性约束 | ✅ 设计定型 | 四层分级：T1 内建 → T2 CDF → T3 自动推断 → T4 CDF-less（显式 opt-in）；优先级：内置 > 项目级 > 用户级 > 推断 > CDF-less |
