...
| `run` 命令入口 | ✅ 设计定型 | `run""` 为命令执行的一等语法，所有命令零前置工作可执行 |
...
| 分级可用性模型 | ✅ 设计定型 | T4 `run`（默认）→ T3 auto-infer → T2 CDF → T1 内建，从下至上渐进升级 |
...
| 命令可用性约束 | ✅ 设计定型 | T4 `run`（默认）→ T3 auto-infer → T2 CDF → T1 内建；运行时自动升级 |
...
| 模块系统 | ✅ 设计定型 | `module ... export` 声明，`import ... with (...)` 导入，`Maybe(*)` 变体导入/导出 |
