...
| 功能 | 状态 | 说明 |
|---|---|---|
| 命令函数抽象 | ✅ 设计定型 | 将 Linux 命令抽象为带签名的函数 |
| CDF 命令描述文件 | ✅ 设计定型 | 定义命令的精确签名 |
| 内置签名库 | ✅ 设计定型 | 核心命令的预置签名 |
...
| 单命令权限注解 | ✅ 设计定型 | 通过 with capabilities 为单个命令指定精确权限集合 |
| 作用域权限声明 | ✅ 设计定型 | 通过 with capability 作用域语法临时授予能力 |
