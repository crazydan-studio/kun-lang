...
| 内建 Primitive 命令 | ✅ 设计定型 | 简单命令（ls/stat/du/df/cp/mv/rm/chmod/chown/mkdir/ln/readlink/free/uname 等）以 Zig 内建实现，进程内执行、无子进程开销、类型精确 |
...
| 命令可用性约束 | ✅ 设计定型 | 无 CDF 的命令不可调用；签名优先级：内置 > 项目级 > 用户级 > 自动推断 |
