...
| [kun-shell.md](kun-shell.md) | Kun Shell 交互式环境（SQLite 日志存储、函数收藏、AST 哈希，未来版本） |
...
- **表达式导向**：所有语句均为表达式，具有返回值（单表达式范式）
- **不可变优先**：数据默认不可变，需要变更时通过显式机制
- **错误显式化**：通过和类型（如 `Result`、`?T`）显式表达所有可能的失败；`Cmd.execSafe` 返回 `Result`，`Cmd.exec`/`Cmd.streamLines`/`Cmd.streamBytes` 失败 panic 或 silent
- **求值策略**：`let in` 立即求值；`Lazy`/`Stream` 显式惰性特区；`Cmd.exec`/`Cmd.execSafe`/`Cmd.streamLines`/`Cmd.streamBytes` 显式执行，无 Command 的 `?`/`!` 后缀（零参函数执行的 `!` 另见[类型系统 - 零参效应函数类型](type-system.md#零参效应函数类型-t-e)）
