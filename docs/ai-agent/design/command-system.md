...
命令名 `<bin>` 直接拼接到 `Cmd.` 之后，Record 选项在前，位置参数在最后：
...
API 签名详见[标准库 Cmd 模块](standard-library.md#cmd--command-工具与命令调用)：

| 函数 | 用途 |
|------|------|
| `Cmd.withEnv` | 添加环境变量 |
| `Cmd.withStdin` | 注入 stdin（字符串或字节流） |
| `Cmd.withRawOpt` | 追加原始 argv token |
...
通过 `kun cmd init` 可从 `man`/`--help` 自动生成命令模块骨架。

## 与标准库的关系

- [标准库 Cmd 模块](standard-library.md#cmd--command-工具与命令调用)：定义 `Cmd` 模块中所有函数的**类型签名**与简短示例
- [系统基线](../architecture/system-baseline.md#命令调用机制)：描述 fork-exec 运行时**实现机制**（系统契约、安全层、内存管理）
- 本文档：定义命令调用的**语法、语义与使用机制**
