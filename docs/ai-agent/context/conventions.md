...
- **Kun 代码验证**：AI Agent 在生成或修改 `.kun` 或 `.cmd.kun` 文件后，必须先运行 `tools/kun-lint.sh check <file>` 检查语法/类型/过时模式，再运行 `tools/kun-lint.sh format <file>` 格式化代码，确认无误后再提交。此步骤由 AI Agent 自动执行，无需用户干预
...
- **代码块标签规范**：文档中的 Kun 代码块只允许使用 ` ```kun ` 标签（当前 Kun 代码）或 ` ```kun-cdf ` 标签（已废弃的 CDF DSL 引用，不提供语法高亮）。不存在 ` ```kun-cmd ` 标签。`.cmd.kun` 命令函数模块代码使用 ` ```kun ` 标签
