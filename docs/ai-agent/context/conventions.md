...
- **Kun 代码验证**：AI Agent 在生成或修改 `.kun` 文件后，必须先运行 `tools/kun-lint.sh check <file>` 检查语法/类型/过时模式，再运行 `tools/kun-lint.sh format <file>` 格式化代码，确认无误后再提交。此步骤由 AI Agent 自动执行，无需用户干预
...
- 项目采用日期版本号：`yyyy.MM.dd`
- 文档版本历史记录在各文件的 `## 版本历史` 节
- 源代码版本通过 Git tag 管理（格式：`v<日期>`）
