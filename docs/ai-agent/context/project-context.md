...
| 推迟项 | Regex 引擎 + Validator → v0.2；DateTime 格式化 → v0.2；沙箱（Landlock/seccomp/rlimit）→ v0.5；Kun Shell → v2.0；等递归类型 → v0.3；Cli 模块 + Parser.Record → v0.3；`Duration`/`Int`/`Float`/`Char` 模块 Primitive 绑定 + PureKun 函数 → v0.2 |
| AI 自治级别 | `implement` |
| 阻塞项 | 无 |
