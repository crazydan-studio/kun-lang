...
1. `do ...` — 结果固定为 `Unit`（单一表达式范式确定）
2. `do ... in` — 返回 `in` 后表达式的值，且可访问 `do` 块内的绑定

`in` 关键字的选择与 `let ... in` 呼应（`syntax.md:433-443`），具有一致性。

**状态**：已于 2026.06.19 通过单一表达式范式设计全面解决——`do` 固定返回 `Unit`，`do in` 用于返回值，`defer` 在 `do` 块退出时执行（先于 `in` 表达式求值）。详见语法设计中「单一表达式范式」及「do 块」章节。
