...
| 示例 | 覆盖范围 |
|------|---------|
| [综合语法：日志文件处理器](file-processor) | `//` 注释 / 字面量前缀 / ADT 空格泛型 / 类型标注 / Lambda 解构 / case / 管道 / do / Record 操作 / 模块导入 / Cmd.pipe / Cmd.<bin>? / f-string |
| [类型系统聚焦](type-showcase) | ADT 四种字段风格 / Newtype / 泛型空格分隔 / Let-多态 / 显式转换 / 类型收窄 / 种类 / Elm 风格函数类型 |
| [IO 与命令调用](networking) | do 块 / Cmd.pipe / Cmd.<bin>? / Signal.on / Port / Pid / SocketAddr / Stream / DateTime / Duration / defer |
