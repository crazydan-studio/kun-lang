...
**结论**：`option <name> "-x" : T` = 可选（`Maybe T`），`option <name> "-x" : T!` = 必填（`T`）。`Bool` 不受 `!` 影响。
...
| 返回值包装 `Result` | 每行独立成功/失败，用户可选择 `filterMap toMaybe` |
...
**结论**：解析器为纯函数，返回值 `Stream (Result T String)`。运行时自动封装到 `CmdResult`。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 0.1.0 | 2026-06-03 | 初始讨论记录：CDF→Kun 代码生成设计全过程 |
