...
**0.2.1 修订决策**：移除行多态，保留扩展积类型

2026-06-07 设计评审后决定**移除行变量和行合一**。原因：

- 行多态的 HM + 行合一 + 效应类型三重复杂度在 Zig 中实现风险过高
- 对于脚本语言的典型使用场景，扩展积类型（编译期字段展开）覆盖了绝大多数"配置扩展"需求
- 函数参数必须使用精确 Record 类型，缺少"至少包含某些字段"的泛化能力，但通过适当设计（基类型 + 扩展类型在函数签名中联动）可缓解

保留：
- 扩展积类型 `{ Base | field : T }`（编译期字段展开）
- Record 更新 `{ r | field = value }`
- 结构化 Record 类型等价

移除：
- 行变量 `a`（`{ a | name : String }` 语法不可用）
- 行合一算法
