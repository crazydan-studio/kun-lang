...
通过 `Cmd.pipe` 函数将多个 `Cmd.*` 调用连接为 OS 管道链，编译为 `pipe2()` + 多次 `fork()`：
...
do
  ...
  // 流式 stdin（大文件——从磁盘惰性读取，逐 chunk 写入子进程）
  fileStream = File.readBytes p"/var/backups/dump.sql.gz"
  Cmd.gunzip {} |> Cmd.stdin fileStream |> Cmd.psql { d = "mydb" }
...
// Cmd.stdin — 注入 stdin（编译器内置，根据实参类型决定字符串或流式模式）
Cmd.stdin : String | Stream Bytes -> Command -> Command
...
`Std` 模块和 `File` 模块中的函数（`File.read`、`File.stat`、`Std.ls` 等）是**进程内同步 syscall**，直接调用 `open()`/`read()`/`fstat()` 等系统调用，不走 fork 子进程。它们**始终立即执行、始终返回 `Result`**，不存在 `?` 变体：

| 维度 | `File.read` / `File.stat` / `Std.ls` | `Cmd.cat`（隐式执行） | `Cmd.cat?`（显式 `?`） |
|------|-------------------------------------|---------------------|----------------------|
  ...
  do
    ...
    case raw of
      Ok text ->
        ...
        case parsed of
          Ok cfg ->
            Std.println f"connecting to {cfg.host}:{cfg.port}"
          Err msg ->
            Std.println f"parse error: {msg}"
      Err _ ->
    ...
    case Parser.Record.toJson cfg of
      Ok text ->
        Std.println text   // → {"host":"localhost","port":8080,"debug":false}
      Err msg ->
...
Kun 运行时在 `do` 块的执行间隙轮询 `signalfd`——信号不中断正在执行的纯函数或 syscall，而是在下一个安全点被读取和处理。安全点包括：`do` 块边界、每个 IO thunk 入口、每次 Command 自动执行返回前、Stream 终端操作消费循环中每 1000 个元素。调度顺序：多个排队信号按接收顺序处理；exit 信号（SIGTERM/SIGINT）在回调完成后立即触发 unwind 和 `defer` 链。`Std.sleep` 在等待期间通过 epoll 同时监听 timerfd 和 signalfd，信号可中断等待。
...
main =
  do
    ...
    List.iter
      (\e ->
        do
          Std.println f"[{e.timestamp}] {e.message}"
      )
      entries
```
...
deploy = \cfg ->
  do
    ...
    Cmd.rsync
      #{ "RSYNC_PASSWORD" = Std.getenv "RSYNC_PWD" ?? "" }
...
| 结构化数据 | `.cmd.kun` + 社区模块 | `Parser.JSON` / `Parser.Record`（`fromJson`/`toJson` + JSON ↔ Record）+ syscall 类型化命令种子 |
