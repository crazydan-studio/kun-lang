...
// 返回 Result，链中任一命令失败 → Err (PipeFailed)
// 若链中仅包含一个命令，行为等价于 Cmd.<bin>?
```
...
- Fork 时机：链中所有子进程在消费时**同时 fork**（通过 pipe2 级联连接），利用 OS 管道缓冲区实现并发执行。消费端阻塞在终点命令的 stdout pipe 上
...
同一 `Command` 值被多次消费是编译错误。`do` 块语句边界的自动执行**仅作用于运行时实际到达的表达式**——`if/case` 中未被选中的分支内的 `Command` 不会执行；提前 `return`/`Process.exit` 后的语句中的 `Command` 也不会执行。
...
do
  ...
  backupCmd = Cmd.tar { czf = "backup.tar.gz" } "src/"
...
> Stream 采用 tagged union 代替函数指针 `{state, next, destroy}`，将双层间接降为单层（`switch(tag)` 替代 `fn_ptr` 调用）。`map`/`take` 等纯参数操作可通过 Zig comptime 融合为单循环；`filter` 等闭包操作至少消除了双层间接。已读取的管道数据不在内存中产生额外副本——缓冲区（8KB）+ 当前元素 + 终端累积结果。
...
Kun 运行时在 `do` 块的执行间隙轮询 `signalfd`——信号不中断正在执行的纯函数或 syscall，而是在下一个安全点被读取和处理。安全点包括：`do` 块边界、每个 IO thunk 入口、每次 Command 自动执行返回前、Stream 终端操作消费循环中每 1000 个元素。调度顺序：多个排队信号按接收顺序处理；exit 信号（SIGTERM/SIGINT）在回调完成后立即触发 unwind 和 `defer` 链。`Time.sleep` 在等待期间通过 epoll 同时监听 timerfd 和 signalfd，信号可中断等待。
...
Landlock 或 seccomp 拦截的调用输出到 stderr，格式：
