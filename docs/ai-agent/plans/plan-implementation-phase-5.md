...
| `code/kun-lang/build.zig` | 新增源文件注册到 build graph（`stream_consumer.zig`/`sha256.zig`/`base64.zig`/`json_parser.zig`/`datetime_fmt.zig`/`glob_engine.zig`/`errors.zig` + 5 primitive 子文件）；添加 zig-regex 依赖 |
...
| `code/kun-lang/src/runtime/regex_engine.zig` | ~600 | NFA 运行时正则引擎（编译+匹配） — 改用 zig-regex 依赖 |
...
**7e. Regex**（使用 zig-regex 依赖）：
- 使用 zig-regex 库（https://github.com/zig-utils/zig-regex）实现正则匹配
- `fromString` → 编译 → Result Regex
- `isMatch`、`firstMatch`、`allMatches`、`replace`、`replaceAll`、`split` 基于 zig-regex
...
| Regex 引擎实现复杂度 | 改用 zig-regex 依赖，不再自研 NFA 引擎；无需预估行数 |
