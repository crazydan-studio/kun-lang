...
**新建文件**：`code/kun-lang/src/runtime/regex_engine.zig`（~600 行）

实现基础 NFA 正则引擎：
- 支持基本元字符：`.` `*` `+` `?` `[]` `()` `|`
- `compile(pattern) !RegexHandle`
...
更新 `crypto.zig:55-61` 的 7 个 Regex 存根为真实调用。
...
| 10 | M7.1–M7.5 | ~1400 行 | M6 |
| 11 | M9.1–M9.8 | ~200 行 | 无 |
| 12 | M10.1–M10.8 | ~150 行 | 无 |

**总预估变更**：~2700 行新增/修改代码，12 个阶段，每阶段独立可验证。
