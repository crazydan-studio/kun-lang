...
本计划将剩余工作组织为 7 个 Step，按依赖关系排序，每步独立可验证。跨 do_block defer unwind 和子进程信号/signalfd 清理属 v0.2 沙箱范围，标记推迟。
...
| 空存根（effect.zig） | **2**（本计划覆盖 2 个，全部已实现） |
| 效应函数未接线 | **10**（见 Step 3） |
...
## Step 6：Primitive 存根替换 — Group C（JSON，Regex/DateTime 推迟 v1.1）

### 6.1 JSON 解析器（基于 std.json）

**修改文件**：`code/kun-lang/src/runtime/primitive/crypto.zig`

使用 Zig 标准库 `std.json` 实现 JSON 解析和序列化：
- `jsonFromStringImpl`：`std.json.parseFromSlice` → 递归将 `std.json.Value` 转换为 Kun `Value`（Object→Map、Array→List、String→string、Number→float、Bool→bool、Null→nil）
- `jsonToStringImpl`：Kun `Value` → `std.json.Value` → `std.json.stringify`

**验证**：`zig build test` 新增 `runtime/test_json.zig`（~10 测试）。

### 6.2 sha256Stream 实现

**文件**：`code/kun-lang/src/runtime/primitive/crypto.zig`
...
| 6 | — | `crypto.zig`, `build.zig`, `test_main.zig` | ~150 | ~10 |
...
Step 6 (JSON) ── JSON Object→Map → 依赖 Step 1 (hash_map.zig)
...
| Regex 引擎（`regex_engine.zig`） | 推迟，评估 zig-regex 或自研 | v1.1 |
| Validator 模块 | 依赖 Regex 引擎 | v1.1 |
| DateTime 格式化（`datetime_fmt.zig`） | 非 MVP 阻塞项 | v1.1 |
...
| 版本 | 变更 |
|------|------|
| 2026.06.24 | R12 审计：无新问题 |
| 2026.06.24 | R11 审计修复（6 项）：Stream 构造点识别修正、JSON→Map 依赖、callEval 用名统一、未绑定变量引用修正、依赖图并行说明、effect 空存根备注 |
| 2026.06.24 | R10 审计：无新问题（全数字一致性校验通过） |
| 2026.06.24 | R9 审计：无新问题 |
| 2026.06.24 | R8 审计修复（3 项）：Primitive 存根计数 29→48、桶描述修正、缺陷计数描述 |
| 2026.06.24 | R7 审计：无新问题 |
