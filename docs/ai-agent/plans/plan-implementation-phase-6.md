...
Phase 1-5 首次审计发现 44 项缺陷，随后修复了 18 项（514 测试全通过）。当前剩余 26 项缺陷，其中 Map/Set 哈希表基础设施缺失是核心阻塞项，效应检查器 3 空存根 + 10 未接线次之，Parser 10 种语法缺失影响语言完整性，29 个 Primitive 存根限制标准库可用性。
...
|------|-----|
| 测试 | **514**（均通过） |
| 剩余缺陷总数 | **26**（本计划覆盖 22 项 + 推迟 4 项） |
| 空存根（effect.zig） | **3**（见 Step 3） |
| 效应函数未接线 | **10**（见 Step 3） |
| Parser 语法缺失 | **10**（见 Step 2） |
| Primitive 存根 | **29**（见 Step 4-6） |
...
**桶结构**（使用普通 `struct`，字段通过手动偏移、`@ptrCast` 和 `@alignCast` 访问 `[*]u8` 原始字节数组，而非 `extern struct`——因为 `extern struct` 不支持 tagged union 字段）：
...
// hash_offset  = 0
// key_offset   = 8
// value_offset = 8 + @sizeOf(Value)
// occ_offset   = 8 + @sizeOf(Value)*2
const MAP_BUCKET_SIZE = ...; // 编译期计算
```
...
- 在 `lbrack` handler 中：解析完第一个表达式后，连续检测两个 `.dot` token
- 两个 `.dot` 后若跟随表达式（非 `]` 或 `,`），识别为范围
- `[1..10]` → `Expr.range_literal`；`[1..10..2]` → step 可选
- **Lexer 修改**：添加 `TokenKind.dot_dot`。当前 lexer 中两个连续 `.dot` 被识别为两个 `.dot` token。修改 `tryReadMultiCharOp` 或添加专门的 `..` 检测
...
- 识别 Stream 构造点：Cmd.<bin>（无 ?/!）、Cmd.pipe、Stream.* 构造器
  - 识别方式：ident 前缀匹配 "Cmd." + 非已知 API 函数，或 "Stream." 前缀
...
- 循环 consumeNext
- 对每个元素：创建 Frame，bind 参数，调用 env.eval_fn(callback.body, frame, allocator)
```
...
| 函数 | 修复 |
|------|------|
| `statImpl` | 调用 `std.fs.cwd().statFile`，填充 size/mode/type/atime/mtime/ctime/uid/gid |
...
- 匹配：NFA 模拟（跟踪活跃状态集）
- 支持：`.` `*` `+` `?` `[]` `()` `|` 转义 `\`
- API：`compile(pattern) !RegexHandle`、`isMatch`、`firstMatch`、`allMatches`、`replace`、`replaceAll`、`split`
...
将 `unbound_variable: []const u8` → `unbound_variable: struct { name: []const u8, span: Span }`，`unbound_type` 同处理。**注意**：当前 `constraint.zig` 中这两个错误变体未被实际 emit（约束生成器对未绑定变量创建新类型变量而非报错）。本修改仅更新类型定义和 i18n 格式，为 Step 2.4（未绑定变量检测）做准备。**同步更新** `i18n.zig` 的消息格式化以使用 span 而非 `"0:0"`。
...
Step 6 (Regex/DateTime/JSON) ── 依赖新建引擎文件（均在 Step 6 自身中创建）

Step 7 (质量加固) ────────── 无依赖
```

Step 1-4 + Step 6 可并行推进（各 Step 触碰不同文件，无冲突）。Step 5 依赖 Step 1 的哈希表模块。Step 7 在最后执行。
