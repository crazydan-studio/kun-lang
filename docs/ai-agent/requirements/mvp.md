...
- 工具：`Cli`（v0.1 仅声明器，parse 推迟 v0.5）、`Validator`、`Parser.JSON`、`Parser.Record`（v0.5）
- 并发：`Task`（v0.5）
...
- `Parser.Record.fromJson` / `Parser.Record.toJson`（编译期代码展开 v0.5）
- `Task.spawn` / `Task.all`（并发 v0.5）
- `Random.*`（需 CSPRNG 系统调用，实现推迟 v0.5；模块签名已在 v0.1 中定义）
- `Sys.ps` / `Sys.free` / `Sys.df`（v0.5）已移除——可通过 `Cmd.ps` / `Cmd.free` / `Cmd.df` 替代，故不纳入语言标准库
- `Test` 模块及 `kun test` 子命令——纯断言函数签名已在标准库中定义但不实现；脚本验证通过直接运行并检查退出码完成（v1.0）
- `kun-shell` 交互式环境（Kun Shell）——设计已定型，实现在 v2.0 前不启动

## MVP 后优先级（v0.2 → v0.5 → v1.0 → v2.0）

| 版本 | 特性 |
|------|------|
| 0.2.0 | 模块系统完整搜索路径 |
| 0.5.0 | Cli.parse + Parser.* + Task.spawn + Random.* + 类型化命令模块 |
