...
- 工具：`Cli`（v0.1 仅声明器，parse 推迟 v0.5）、`Validator`、`Parser.JSON`（v0.5）、`Parser.Record`（v0.5）、`Test`
- 并发：`Task`（v0.5）
...
|------|------|
| v0.2 | 模块系统完整搜索路径 + 泛型 |
| v0.5 | Cli.parse + Parser.* + Task.spawn + Sys.ps/free/df + Random.* + 类型化命令模块 |
| v1.0 | Cmd.timeout/retry/withRunAs + Signal.on + 性能优化 |
