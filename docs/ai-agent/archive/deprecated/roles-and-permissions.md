...
> **本设计已废弃**。`with caps` 能力声明方案已被 CLI 参数（`--allow-path` / `--allow-net` / `--no-sandbox`）方案完全替代。
> 安全隔离设计见 `../architecture/system-baseline.md` 安全隔离章节。本文件保留仅作历史参考，不应用于新开发。
...
| 网络监听 | 无 | 禁止监听任何端口 |
| 进程执行（`run`） | 无 | `run""` 子进程执行需 `process.run` 白名单；`.cmd.kun` 命令函数通过 `.cmd.kun` 存在授权 |
...
**`chdir` 操作**：Kun 不提供内建的 `chdir` 函数。脚本不应在运行时切换工作目录。若确实需要改变进程的 CWD（如影响子进程的路径解析），需通过 `run` 或 `.cmd.kun` 命令函数调用外部 `cd` 命令。
...
对外部命令的权限元数据修改（如 `chmod`、`chown`）通过内建 Primitive 授权，`fs.write` 沙箱控制可修改的路径范围。

#### 环境变量过滤（子进程执行前安全策略）

命令函数执行子进程前，运行时自动过滤环境变量，防止密钥泄漏和注入攻击：
...
| **沙箱**（namespace + seccomp） | 命令函数执行的子进程 | **尽力而为**——受内核版本和动态链接限制 | 子进程以 OS 用户权限运行，Kun 无法完全约束其内部文件访问 |
...
1. **子进程文件访问**：命令函数执行的子进程以 OS 用户权限运行。对动态链接的命令，mount namespace 需暴露系统库路径（`/usr`、`/lib`），间接暴露整个根文件系统。`capability_check` 不控制子进程内部行为——它只在启动前检查传参（`cat p"/etc/shadow"` 中的路径），子进程内的文件访问由 OS 和沙箱策略约束。
...
- 一次沙箱配置对应一次外部命令调用
- `.cmd.kun` 不包含独立的能力行为声明，seccomp 规则由 Builder 调用链推导
...
各层的信任假设：第 2 层（`capability_check`）是唯一不被其他层依赖的硬防线。第 3 层（沙箱）和第 4 层（命令签名）提供纵深防御，但已知有局限，不可单独依赖于它们保障安全。
...
| 0.1.1 | 2026-05-31 | 深化：能力类型目录、运行时检查架构、动态授予、父-子传递、威胁模型 |
| 0.2.0 | 2026-06-01 | **重写**：全新 `with caps` 语法、零默认能力、移除 CDF 能力声明、能力对象编译器内置、最低 Linux 内核 3.8、独立资源预算限流层、模块禁止声明能力、函数级 `with caps ... do` 交集收窄、`--audit`/`--confirm`/`--cap-log` 审查机制 |
