...
> **本设计已废弃**。`.cmd.kun` + Builder API + 幻影类型 + 编译器封装置方案已被 `Cmd.<bin>` 语法 + Record 选项 + 自动模块发现方案完全替代。
> 新命令调用系统的设计见输入记录 `docs/ai-agent/input/input-architecture-redesign.md` 第 3-4 节。
> 本文件保留仅作历史参考，不应用于新开发。
