...
load_module(path):
    ...
    for each import in module.imports:
        load_module(resolve_path(import))  // 递归
...
// 内置操作函数表
typedef struct {
    const char* module_name;      // 模块名（如 "List"）
    const char* func_name;        // 函数名（如 "map"）
    void*       fn_ptr;          // 函数入口指针
    uint64_t    arity;            // 参数个数
    TypeSignature* signature;     // 类型签名
} PrimitiveEntry;
...
fn args_parse_raw(env: void*, args: void*) -> void* {
    let decls = *(Array*)args[0];   // 声明器列表
    let raw   = *(Array*)args[1];   // 原始参数列表
...
typedef struct {
    int32_t   fd;          // 文件描述符
    uint8_t   buffer[8192]; // 读取缓冲区
    uint64_t  buf_pos;     // 缓冲区当前位置
    uint64_t  buf_end;     // 缓冲区有效数据长度
    bool      eof;         // 文件是否已读完
    Arena*    arena;       // 行字符串的分配器
} ReadLinesState;
