...
当前 `docs/architecture/system-baseline.md` 仅包含运行时的骨架描述：技术栈、命令加载机制高层面板、错误诊断类型、安全模型概览、类型系统概述、命令签名系统概述。缺少核心模块的深化设计。
...
| 文件 | 说明 |
|------|------|
| `docs/architecture/system-baseline.md` | 从骨架文档扩展为完整的运行时架构设计文档 |
...
| 文件 | 说明 |
|------|------|
| `docs/architecture/index.md` | 更新文件说明 |
| `docs/architecture/module-boundaries.md` | 同步运行时新增组件的模块边界 |
| `docs/context/project-context.md` | 更新活跃工作状态 |
| `docs/backlog/index.md` | 更新运行时架构设计状态 |
...
- 更新 `docs/architecture/index.md` 文件说明
- 同步更新 `docs/context/project-context.md` 活跃工作
- 更新 `docs/backlog/index.md` 为 `in-progress`

## 验证方法

1. **构建验证**: `cd /workspace/docs && pnpm lint:md && pnpm build`
2. **一致性审查**: 逐项对照 `docs/design/syntax.md` 确认所有代码示例语法正确（无 `<>`、`--`、`::`、`=>` 等废弃语法）
3. **类型表示审查**: 逐项对照 `docs/design/type-system.md` 确认运行时类型表示与类型系统设计一致
4. **标准库审查**: 逐项对照 `docs/design/standard-library.md` 确认标准库集成部分与 API 签名一致
5. **边界审查**: 确认新引入的运行时概念与 `docs/architecture/module-boundaries.md` 的模块划分一致，必要时更新该文档
...
| 设计决策与之前语法/类型系统矛盾 | 实施前确认所有代码示例和类型签名与 `docs/design/syntax.md`、`docs/design/type-system.md` 一致 |
...
1. 所有运行时类型表示是否与 `docs/design/type-system.md` 一致
2. 所有代码示例和伪代码中的语法是否与 `docs/design/syntax.md` 一致（无废弃语法）
3. 命令加载机制的 C ABI 设计是否与 Zig 的 FFI 能力兼容
4. IO 效果编排模型是否与 `do`/`do in`/`<-` 语法语义一致
5. 内存管理策略是否与"不可变默认"的设计原则一致
6. 模块解析策略是否与 `import` 语法语义一致
7. 标准库集成是否与 `docs/design/standard-library.md` 的 API 签名一致
