...
cat("/etc/nginx/nginx.conf") with capabilities fs.read("/etc")
...
cat("/etc/config") | grep("pattern") with capabilities fs.read("/etc")
```
...
with capability net.http("api.example.com") {
  let response = curl("https://api.example.com/data")
  process(response)
}
...
with capability fs.read("/etc"), net.http("*") {
  # 可读写 /etc 和访问任意网络
  let config = cat("/etc/app/config.kun")
...
cat("/var/log/app.log") | sort() with capabilities fs.read("/var/log"), fs.write("/tmp")

# 将 cat 输出重定向到文件：需要右侧命令的写入权限
# 此处 write_file 需要 fs.write("/tmp")
cat("/var/log/app.log") | write_file("/tmp/filtered.log") with capabilities fs.read("/var/log"), fs.write("/tmp")
```
