...
`IO<T>` 与 `Maybe<T>`、`Result<T, E>` 在类型标注中均写作 `Identifier<T>` 形式，但二者属不同类型分类：
...
1. **简化类型标注**——`IO<String>` 比引入一套全新效应标记语法（如 `String ! IO` 或 `String +IO`）更直观，用户只需理解"尖括号内是结果类型"这一规则
2. **效应类型极简**——Kun 不做完整代数效应系统（algebriac effects），`IO<T>` 只是一个"副作用边界标记"，不涉及效应多态或效应处理器，不值得为其设计独立语法家族
3. **Haskell 先例**——Haskell 的 `IO String` 与 `Maybe String` 写法完全一致，不影响开发者对 IO 特殊性的理解；IO 的独特性通过类型系统的特殊规则（纯函数不可调用 IO 函数）而非语法来体现

区分方法：查看类型签名时，`IO` 关键词直接提示"此函数有副作用"——这是类型分类信息，不依赖语法差异。
