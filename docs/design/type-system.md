...
case fileType p of
  ...
  Ok _           -> fail "unsupported input type"
  Err err        -> fail f"cannot access: {err}"
```
...
3. **消除转换摩擦**：无 `toList`/`fromList` 转换，所有序列操作在同一类型上链式完成
...
- 每个变体可携带 0 个或多个字段
- 变体名称必须以大写字母开头
- 变体字段可以是无名（元组风格或空格分隔）或具名（Record 风格）
- 泛型参数使用空格分隔：`type Maybe t = Just t | None`
...
Kun 采用无约束的参数化多态（类似 Elm，无 Typeclass/Trait），泛型参数以空格分隔：
...
- 嵌套泛型用括号分组：`IO (Result FileType IOError)`
